...
public class VodDao {
	...
	public List<VodSubjectModel> getListSubjectVod(int langId, String type) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.GET_LIST_VOD_SUBJECT, params, "langid,type", outParam.size() / 8);
		for (int i = 0; i < outParam.size(); i = i + 8) {
			VodSubjectModel vod = new VodSubjectModel();
			vod.setId(outParam.get(i));
			vod.setName(UnicodeConverter.decodeUnicode(outParam.get(i + 1)));
	...
	public int addVodSubject(VodSubjectModel vod) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.ADD_VOD_SUBJECT, params, "name,image,imageic,invisible,type", rs);
		return rs;
	}
	...
	public int editVodSubject(VodSubjectModel vod) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.EDIT_VOD_SUBJECT, params, "idsubject,name,image,imageic,invisible,type", rs);
		return rs;
	}
	...
	public int deleteVodSubject(int idSubject) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.DELETE_VOD_SUBJECT, params, "idsubject", rs);
		return rs;
	}
	...
	public List<VodContentModel> getListContent(VodContentModel vod) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.GET_LIST_VOD_CONTENT, params, "idsubject,langid", outParam.size() / 15);
		for (int i = 0; i < outParam.size(); i = i + 15) {
			...
			con.setName(UnicodeConverter.decodeUnicode(outParam.get(i + 2)));
			con.setProductor(UnicodeConverter.decodeUnicode(outParam.get(i + 3)));
			con.setDirector(UnicodeConverter.decodeUnicode(outParam.get(i + 4)));
			con.setActor(UnicodeConverter.decodeUnicode(outParam.get(i + 5)));
	...
	public int addNewMovie(VodContentModel con) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.ADD_VOD_CONTENT, params,
	...
	public int editMovie(VodContentModel con) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.EDIT_VOD_CONTENT, params,
	...
	public int deleteMovie(int idContent) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.DELETE_VOD_CONTENT, params, "idcontent", rs);
		return rs;
	}
	...
	public int insertVodRemote(VodPercentModel per) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.INSERT_VOD_REMOTE_PERCENT, params, "filename,uuid,title", rs);
		return rs;
	}
	...
	public int deleteVodRemotePercent(String uuid) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.DELTE_VOD_REMOTE_PERCENT, params, "uuid", rs);
		return rs;
	}
	...
	public List<VodPercentModel> getListVodRemotePercent() {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.GET_VOD_REMOTE_PERCENT, params, "none", outParam.size() / 3);
	...
	public List<HashMap<String, String>> getSubtitle(int vodId) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.GET_SUBTITLE, params, "vodId", outParam.size() / 3);
		for (int i = 0; i < outParam.size(); i = i + 3) {
	...
	public int addOrEditSubtitle(int vodId, String listSub) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.ADD_OR_EDIT_SUBTITLE, params, "vodId,listSub", rs);
		return rs;
	}
	...
	public static void main(String[] args) {
		System.out.format("%,d%n", 1000);
		String n = NumberFormat.getInstance().format(1000);
