...
public class serviceImage extends ServiceParent {
	...
	private void exportMonth(HttpServletRequest request,
		...
		try {
			...
			Date date = new Date();
			String filename = "";
			filename = date.getMinutes() + date.getHours() + "-"
					+ date.getDate() + "-" + date.getMonth() + "-"
					+ date.getYear() + ".xls";
			WritableWorkbook workbook = Workbook.createWorkbook(new File(path1
					+ filename));
			...
			Label label = new Label(0, 0,
					readerLang.getContent("static_month"), cellFormat);
			sheet.addCell(label);
			...
			label = new Label(0, 4, readerLang.getContent("Monthly"),
					cellFormatcol);
			sheet.addCell(label);
			label = new Label(1, 4, readerLang.getContent("numberl_service"),
					cellFormatcol);
			sheet.addCell(label);
			...
			for (int i = 0; i < v_rs.size(); i++) {
				...
				label = new Label(1, (i + 5), String.valueOf(v_rs.get(i)
						.getQuanlity()), cf2);
				sheet.addCell(label);
			}
	...
	@SuppressWarnings("deprecation")
	private void exportGenres(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			HttpSession session = request.getSession();
			ReaderLang readerLang = new ReaderLang();
			...
			cell.setCellFormat(cellFormat);
			Label label = new Label(0, 0,
					readerLang.getContent("static_genres"), cellFormat);
			sheet.addCell(label);
			...
			sheet.addCell(label);
			VideoReport rmi = new VideoReport();
			Vector<DataReport> v_rs = rmi.getUsedFrequency_gener(from, to,
					LangID);
	...
	@SuppressWarnings("deprecation")
	private void exportStaticfilm(HttpServletRequest request,
			HttpServletResponse response) {
		HttpSession session = request.getSession();
		ReaderLang readerLang = new ReaderLang();
		if (session.getAttribute(Def._lang) != null) {
		...
		try {
			...
			cell.setCellFormat(cellFormat);
			Label label = new Label(0, 0,
					readerLang.getContent("static_of_movies"), cellFormat);
			sheet.addCell(label);
			WritableCellFormat cellFormatcol = new WritableCellFormat();
			cellFormatcol.setBorder(Border.ALL, BorderLineStyle.THIN);
			cellFormatcol.setAlignment(Alignment.CENTRE);
			...
			label = new Label(0, 3, "From " + from + " - To " + to,
					cellFormatcol);
			sheet.addCell(label);
			label = new Label(0, 6, readerLang.getContent("movies_name"),
					cellFormatcol);
			sheet.addCell(label);
			label = new Label(1, 6, readerLang.getContent("Price"),
					cellFormatcol);
			sheet.addCell(label);
			label = new Label(2, 6, readerLang.getContent("numberl_service"),
					cellFormatcol);
			sheet.addCell(label);
			label = new Label(3, 6, readerLang.getContent("Amount"),
					cellFormatcol);
			sheet.addCell(label);

			VideoReport rmi = new VideoReport();
			Vector<DataReport> v_rs = rmi.getUsedFrequency_name(from, to,
					LangID, -1, -1);
			for (int i = 0; i < v_rs.size(); i++) {
				WritableCellFormat cf1 = new WritableCellFormat();
				cf1.setBorder(Border.ALL, BorderLineStyle.THIN);
				...
				sheet.addCell(label);
				label = new Label(2, (i + 7), String.valueOf(v_rs.get(i)
						.getQuanlity()), cf1);
				sheet.addCell(label);
				label = new Label(3, (i + 7), String.valueOf(v_rs.get(i)
						.getAmount()), cf1);
				sheet.addCell(label);
			}
			if (v_rs.size() > 1) {
				...
				font.setBoldStyle(WritableFont.BOLD);
				cf3.setFont(font);
				Formula formua = new Formula(3, v_rs.size() + 7, "SUM(D8,D"
						+ (v_rs.size() + 7) + ")", cf3);
				sheet.addCell(formua);
				formua = new Formula(2, v_rs.size() + 7, "SUM(C8,C"
						+ (v_rs.size() + 7) + ")", cf3);
				sheet.addCell(formua);
	...
	@SuppressWarnings("deprecation")
	private void staticroomservice(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			HttpSession session = request.getSession();
			ReaderLang readerLang = new ReaderLang();
			...
			cell.setCellFormat(cellFormat);
			Label label = new Label(0, 0,
					readerLang.getContent("static_roomservice"), cellFormat);
			sheet.addCell(label);
			...
			label = new Label(0, 3, "From " + from + " - To " + to,
					cellFormatcol);
			sheet.addCell(label);
			label = new Label(0, 5, readerLang.getContent("roomcode"),
					cellFormatcol);
			sheet.addCell(label);
			label = new Label(1, 5, readerLang.getContent("numberl_service"),
					cellFormatcol);
			sheet.addCell(label);
			...
			for (int i = 0; i < v_rs.size(); i++) {
				...
				cf2.setBorder(Border.ALL, BorderLineStyle.THIN);
				cf2.setAlignment(Alignment.CENTRE);
				label = new Label(1, (i + 6), String.valueOf(v_rs.get(i)
						.getQuanlity()), cf2);
				sheet.addCell(label);
			}
			String t = readerLang.getContent("note_report");
	...
	private void exportTransport(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			HttpSession session = request.getSession();
			ReaderLang readerLang = new ReaderLang();
			...
			cell.setCellFormat(cellFormat);
			Label label = new Label(0, 0,
					readerLang.getContent("transportstatic"), cellFormat);
			sheet.addCell(label);
			WritableCellFormat cellFormatcol = new WritableCellFormat();
			cellFormatcol.setBorder(Border.ALL, BorderLineStyle.THIN);
			cellFormatcol.setAlignment(Alignment.CENTRE);
			...
			font.setBoldStyle(WritableFont.BOLD);
			cellFormatcol.setFont(font);
			label = new Label(0, 4, readerLang.getContent("Name"),
					cellFormatcol);
			sheet.addCell(label);
			label = new Label(1, 4, readerLang.getContent("numberl_service"),
					cellFormatcol);
			sheet.addCell(label);
			OrderReport rmi = new OrderReport();
			Vector<DataReport> v_rs = rmi
					.getTransportationRpt(from, to, LangID);
			for (int i = 0; i < v_rs.size(); i++) {
				WritableCellFormat cf1 = new WritableCellFormat();
				cf1.setBorder(Border.ALL, BorderLineStyle.THIN);
				cf1.setAlignment(Alignment.CENTRE);
