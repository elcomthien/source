...
public class ServiceTranport extends ServiceParent {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case 3:// show form detail url
		{
			...
			this.showJSPpage(request, response,
					"/pmsMng/transport/detailURL.jsp");
			break;
		}
		case 4:// detailGround.jsp
		{
			...
			this.showJSPpage(request, response,
					"/pmsMng/transport/detailGround.jsp");
			break;
		}
		case 5:// get content
		{
		...
		case 6:// delete ground
		{
			...
			while (request.getParameter("id" + i) != null) {
				id = Integer.parseInt(request.getParameter("id" + i).toString()
						.trim());
				list.add(id);
				i++;
			}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case 1:// insert ground
		{
			...
			menu.setUrlBg(config._transportation + "/" + icon);
			menu.setUrlImage(config._transportation + "/" + image);
			int t = transport.addItemMenu(id, menu);
			if (t > 0) {
				ManagerFile file = new ManagerFile();
				String path1 = config._temp + "/" + image;
				String path2 = config._pathImage + config._transportation + "/"
						+ image;
				file.copy(path1, path2);
				transferImageFLC.transferImageMain(path1, config._transportation);
				file.deletefile(path1);
				path1 = config._temp + "/" + icon;
				path2 = config._pathImage + config._transportation + "/" + icon;
				file.copy(path1, path2);
				transferImageFLC.transferImageMain(path1, config._transportation);
				file.deletefile(path1);
			}
		...
		case 3:// insert url
		{
			...
			item.setName(name);
			item.setUrlLink(url);
			item.setUrlImage(config._transportation + "/" + image);
