...
public final class DataHelper
{
	...
	//Region - ConvertText
	private static String convertXposString(String source) {
		HashMap<String, String> mapLib = new HashMap<String, String>();
		mapLib.put("U+0026", "&");
		mapLib.put("U+003C", "<");
		mapLib.put("U+003E", ">");
		
		Set<String> keySet = mapLib.keySet();
		for(String key : keySet) {						
			source = source.replace(key, mapLib.get(key));			
		}
		return source;
	}
	
	public static String RebuildMessage(String message, boolean isJson) {		
		if(isJson) {
			message = message.replace("'", "\"");
		}
		message = convertXposString(message);
		return message;
	}
	
	public static String RebuildMessage(String message) {
		return RebuildMessage(message, true);
	}
	...
	public static String getIpRemote()
	{
		...
		try {
			...
			System.out.println("Your current HostAddress : " + hostname);

		} catch (UnknownHostException e) {

			e.printStackTrace();
		}	  
		return hostname + " => Ver: " + Ver;	  
	} 
	...
	public static void main(String[] args)
	{
		...
		/*createFile("Config\\Layout.xml","<?xml version='1.0' encoding='UTF-8'?>\r\n" + 
				...
				"</Layout>\r\n");
		*/
		String s  = "so the message I send to you will be \"Mr. U+0026 Mrs. then U+003C or U+003E. then Mr. U+0026 Mrs. then U+003C or U+003E.\"";
		String r = DataHelper.convertXposString(s);
