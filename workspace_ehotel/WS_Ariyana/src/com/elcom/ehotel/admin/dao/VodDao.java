...
public class VodDao {
	...
	public int addVodSubject(VodSubjectModel vod) {
		...
		params.add(in);
		in = new SubProParam(new String(vod.getIndex()), 0);
		params.add(in);
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.ADD_VOD_SUBJECT, params, "name,image,invisible,type,index", rs);
		return rs;
	}
	...
	public int editVodSubject(VodSubjectModel vod) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.EDIT_VOD_SUBJECT, params, "idsubject,name,image,invisible,type,index", rs);
		return rs;
	}
