...
public class UserManagerDao {
	...
	public List<UserManagerModel> getListUser() {
		...
		for (int i = 0; i < outParam.size(); i = i + 7) {
			...
			user.setRoleid(outParam.get(i + 6));
			list.add(user);
		}
		return list;
	}
	...
	public int addUser(UserManagerModel user) {
		...
		params.add(in);
		in = new SubProParam(new String(user.getRoleid()), 0);
		params.add(in);
		...
		LogUtil.logDao(UserManagerDao.class.toString(), SQL.ADD_USER, params, "user,pass,name,address,department,active,role", rs);
		return rs;
	}
	...
	public int editUser(UserManagerModel user) {
		...
		LogUtil.logDao(UserManagerDao.class.toString(), SQL.EDIT_USER, params, "id,name,address,department,active,roleid", rs);
		return rs;
	}
	...
	public int changePassAdmin(String id, String newpass) {
		...
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.CHANGE_PASS_ADMIN, params);
			if ((params != null) & (params.size() > 0)) {
		...
		LogUtil.logDao(UserManagerDao.class.toString(), SQL.CHANGE_PASS_ADMIN, params, "id,newpass", rs);
		return rs;
	}
	...
	public List<HashMap<String, String>> getListRold() {
		...
		LogUtil.logDao(UserManagerDao.class.toString(), SQL.GET_LIST_ROLE, params, "none", outParam.size() / 2);
		for (int i = 0; i < outParam.size(); i += 2) {
			HashMap<String, String> map = new HashMap<String, String>();
			map.put("id", outParam.get(i));
			map.put("value", outParam.get(i + 1));
			list.add(map);
		}
		return list;
	}
