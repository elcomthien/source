...
public class ServiceLiveTVMain extends ServiceParent {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case 14: {
			...
			deleteFileVideoAdvertise(filename);
			boolean flag = LiveTvServiceDBI.deleteChannelAdvertise(id);
			if (flag)
				out.print("success");
			else
				out.print("failed");
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case 4: { // add channel
			...
			String name = getFileNameByDateTime() + filetype;
			boolean temp = addChannelFileToServer(filename, name);
			if (temp) {
				ConfigLoader loader = new ConfigLoader();
				Config config = loader.getConfig();
				String channellink = name;
				boolean flag = LiveTvServiceDBI.addChannelForHotel(channelname, channellink, image);
				if (flag) {
		...
		case 6: {
			...
			for (int i = 0; i < arr.length; i = i + 2) {
				String channelid = arr[i].replace("c", "");
				String channelcode = arr[i + 1];
				LiveTvServiceDBI.updateChannelCode(channelid, channelcode);
			}
			if (flag) {
				out.print("true");
			} else {
	...
	public void deleteFileVideoAdvertise(String filename) {
		System.out.println("Delete file on server: " + filename);
		ManagerFile file = new ManagerFile();
		String path = "/data/media/Video/" + filename;
	...
	@SuppressWarnings("unused")
	private boolean download_file_channel(String path_, String file_name) {
		String host = "";
		int port = 21;
		String user = "";
		String pass = "";
		...
		boolean flag = true;
		// String server = "10.206.1.67";
		try {
			transferVod(path_, file_name, "", host, port, user, pass); // chỗ
																		// này
																		// là để
																		// insert
																		// video
		...
		/*
		 * FTPClient ftpClient = new FTPClient(); try { ftpClient.connect(host, port); ftpClient.login(user, pass);
		 * ftpClient.enterLocalPassiveMode(); ftpClient.setFileType(FTP.BINARY_FILE_TYPE); String remoteFile1 =path_;
		 * 
		 * File downloadFile1 = new File("/data/loop_video/"+file_name); System.out.println("downloadFile1 : "+downloadFile1); OutputStream
		 * outputStream1 = new BufferedOutputStream(new FileOutputStream(downloadFile1.getAbsolutePath())); boolean success =
		 * ftpClient.retrieveFile(remoteFile1, outputStream1);//storeFile outputStream1.close(); if (success) {
		 * System.out.println("Downloaded file is successfully."); }else{ System.out.println("Downloaded file is not successfully.");
		 * flag=false; } }catch(Exception e){ System.out.println(e.toString()); }finally { try { if (ftpClient.isConnected()) {
		 * ftpClient.logout(); ftpClient.disconnect(); } } catch (IOException ex) { ex.printStackTrace(); } }
		 */
		return flag;
	}

	public boolean transferVod(String filepathSrc, String filenameServer, String nameview, String hostSrc, int portSrc, String userSrc, String passSrc) {
		boolean flag = true;
		String hostserver = "10.206.3.5";
		String foldername = "/data/loop_video/";
		System.out.println("filepathserver--------------------------: " + foldername);
		try {
			// cau hinh rmi
			...
			System.out.println("file path inserver = = = = = " + foldername + filenameServer);
			UUID id = ftpgateway.download(server, hostserver, foldername + filenameServer, 5000);
			System.out.println("id uuid = = = " + id);
			// String file =
			// filenameServer.substring(filenameServer.lastIndexOf(".") - (-1));
			/*
			 * if ("mp4".equalsIgnoreCase(file) || "mkv".equalsIgnoreCase(file) || "mov".equalsIgnoreCase(file) ||
			 * "flv".equalsIgnoreCase(file) || "avi".equalsIgnoreCase(file)) if (id != null) { nameview = nameview.replaceAll("'", " i"); }
			 * else { int vodId = Integer.parseInt(filenameServer.substring(0, filenameServer.lastIndexOf("."))); return false; }
			 */
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
	...
	public boolean addChannelFileToServer(String filename, String name) {
		boolean flag = true;
		...
		try {
			...
			FTPServerStruct server = new FTPServerStruct(arr[0], Integer.parseInt(arr[1]), arr[2], arr[3], filename);
			System.out.println("server = = = " + server);
			UUID id = ftpgateway.download(server, "localhost", "/data/media/Video/" + name, 5000);
			System.out.println("uuid = = = " + id);
			String percent = "";
			String temp = "";
			while (ftpgateway.transfered(id) < ftpgateway.fileSize(id)) {
				temp = ftpgateway.transfered(id) * 100 / ftpgateway.fileSize(id) + "%";
	...
	public String getChannelCode() {
		...
		list = LiveTvServiceDBI.getChannelCode();
		String mData = "";
		mData += "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\" ?>\n";
		mData += "<xml>";
		for (int i = 0; i < list.size(); i++) {
			mData += "<Item>";
			mData += "<name>";
			mData += "<![CDATA[" + list.get(i).get("channelname") + "]]>";
			mData += "</name>";
			mData += "<id>";
			mData += list.get(i).get("channelid");
			mData += "</id>";
			mData += "<code>";
			mData += list.get(i).get("channelcode");
			mData += "</code>\n";
			mData += "</Item>";
		}
		mData += "</xml>";
	...
	public boolean updateChannelCode(String channelid, String channelcode) {
		return true;
	}

	public static void main(String[] args) {
		ServiceLiveTVMain s = new ServiceLiveTVMain();
		System.out.println(s.getChannelCode());
	}
}