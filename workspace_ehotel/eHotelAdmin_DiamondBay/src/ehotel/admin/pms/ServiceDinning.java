...
public class ServiceDinning extends ServiceParent {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case 7:// detail restaurant
		{
			...
			this.showJSPpage(request, response,
					"/pmsMng/Dinning/detailRetaurent_Webeditor.jsp");
		...
		case 13:// detail restaurant
		{
			...
			if(item!=null){
			response.setContentType("text/xml");
			String value_Des=getDes_Retaurent(item);				
				out.write(value_Des);
			}			
			break;
		}
...
//				int id = dinning.addDiningMenu(parent, menu);
				int id = pmsServiceDBI.addMenuDining(parent, subjectName, image, urlBg);
				if (id > 0) {
		...
		case 7:// insert item restaurant
		{
			...
			int t = pmsServiceDBI.addItemDining(menuid, name, def, price,
					price_large, price_small, config._dining +"/"+ url, unit);
			if (t > 0) {
				ManagerFile file = new ManagerFile();
...
//		int id = pmsServiceDBI.addMenuDining(parent, subjectName, image, urlBg);
	...
	private String getDes_Retaurent(eRestaurant e) {				
			String mData = "";
			mData += "<?xml version=\"1.0\" encoding=\"ISO-8859-1\" standalone=\"yes\" ?>\n";
			mData += "<xml>";	
				mData += "<Item>";
				mData += "<des>";
				mData += "<![CDATA[" + e.getDef() + "]]>";
				mData += "</des>";
				mData += "</Item>";	
			mData += "</xml>";
			return mData;
