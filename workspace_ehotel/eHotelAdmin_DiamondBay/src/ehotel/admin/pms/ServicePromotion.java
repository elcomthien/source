...
public class ServicePromotion extends ServiceParent {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case 2:// show form detail
		{
			...
			if (!link.equals("")) {
				link = link.replace("ehotel.elcom.tv", config._HostFtpServer);
				item.setLinkWeb(link);
			}
			request.setAttribute("eItem", item);

			this.showJSPpage(request, response,
					"/pmsMng/Other/detailPromotion.jsp");
			break;
		}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case 1:// insert
		{
			...
			if (b > 0) {
				...
				String path = ServicePromotion.class.getResource("").getPath();
				path = path.substring(0, path.indexOf("webapps") + 8)
						+ "Promotion/";
				String linkpro = path + b + type;
				file.copy(linkpath, linkpro);

				String pathhtml = path + "Promotion.html";
				String pathpro = path + b + ".html";
				file.copy(pathhtml, pathpro);

				String path1 = config._temp + "/" + image;
				String path2 = config._pathImage + config._promotion + "/"
						+ image;
	...
	private String getContent(int index, int page) {
		...
		for (int i = 0; i < info.size(); i++) {
			...
			if (item.getContent() != null) {
				mData += "<![CDATA["
						+ UtilString.converString(item.getContent()) + "]]>";
			} else {
				mData += "<![CDATA[]]>";
			}
			mData += "</Des>";
	...
	public static void main(String[] args) {
		String path = "/ehotel/service/app/apache-tomcat-6.0.29/webapps/Promotion/Promotion.html";
		path = path.substring(0, path.indexOf("webapps") + 8);
