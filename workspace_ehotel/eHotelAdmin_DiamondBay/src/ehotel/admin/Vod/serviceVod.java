...
public class serviceVod extends ServiceParent {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case 12:// change status vod subject service
		{
			...
			boolean flag = true;
			if (invisible == 0)
				flag = vodContentServiceDBI.updateInvisibleVodSubjectService(
						subId, 1);
			else if (invisible == 1)
				flag = vodContentServiceDBI.updateInvisibleVodSubjectService(
						subId, 0);
	...
	private String searchVodSrv(int subId, int index, int page, String text) {
		...
		// }
		int count = SrvMod.countVodOfSubject(subId);
		List<Vod> vod = vodContentServiceDBI.searchVodSrv(subId, text, LangID,
				fr, to);
		String mData = "";
		mData += "<?xml version=\"1.0\" encoding=\"ISO-8859-1\" standalone=\"yes\" ?>\n";
		mData += "<xml count='" + count + "'>";
