...
//		my.loadcomplet(me.obj.Id);
	...
	ctTable.fucsendmessage = function() {
		...
		f.complet = function(tx) {
			...
			initTinymce(2, "650px");			
			//new nicEditor({buttonList : ['bold','italic','underline']}).panelInstance('messagecontent');
			//nicEditors.findEditor( "messagecontent" ).setContent("");
	...
	this.acceptmessage = function(id) {
		...
		var content_insert =getDataFromEditor("messagecontent");		
		/*alert(content_insert);*/
		if (subject.length != 0 && sender.length != 0) {	
			...
			param += "&content=" + content_insert;			
			//param += "&content=<div>Dear Mr/Mrs</div><div><br></div><div>Warmest greetings from Vinpearl Luxury Nha Trang!</div><div><br></div><div>Let us take opportunity to express our sincere appreciation to you for choosing Vinpearl Luxury Nha Trang as your holiday destination. We hope you ae enjoying your time with us.</div><div><br></div><div>It is my hornor to invite you and your family to join me for a Sunset Drink from 17h00 to 17h30 on Tuesday 18th August,2015.</div><div>I am very pleased to welcome you at wave bar to have a share time together. I will be very appreciated your comments to Vinpearl Luxury Nha Trang. Vinpearl always treasure you as our valued guest and will continue to enhance our facilities and add on more for your enjoyment and selection respectively.</div><div><br></div><div>Should need any further information,please do not hesitate to contact us at extention<div>Best Regards</div><div>Brett J Burton</div><div>General Manager</div><div>Vinpearl Luxury Nha Trang</div> <div>Dear Mr/Mrs</div><div><br></div><div>Warmest greetings from Vinpearl Luxury Nha Trang!</div><div><br></div><div>Let us take opportunity to express our sincere appreciation to you for choosing Vinpearl Luxury Nha Trang as your holiday destination. We hope you ae enjoying your time with us";
			var f = new AjaxPostText(url, param);
			f.complet = function(tx) {
				$("div_layer_vitual").style.display = "none";
				my.reload();
	...
	table.dataBind = function() {
		...
		html += langpms.no;
		html += "</th>";
		html += "<th align='left' valign=\"middle\">";
		html += langpms.room;
		html += "</th>";
		html += "<th align='left'  valign=\"middle\">";
...
function contextTableEX() {
	...
	this.draw = function(x, y) {
		...
		if (my.obj.XXX == "0") {
			html += "<li class='icon_visibility1'  style='display: none;'>";
