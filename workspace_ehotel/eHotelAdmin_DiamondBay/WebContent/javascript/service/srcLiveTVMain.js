...
var ctChannel = new contextChannel;
var my = this;
var tempId = 0;
...
function renderEventGroup() {
	var ul = $("menutree").getElementsByTagName("ul");
...
function showfromAddMenu() {
	closeMenu();
	var textbox = new newBox(NewSubject, "", "", _titleName, _Image, ok, cancel);
	textbox.show();
...
function deleteMenuSubject(groupId) {
	closeMenu(0);
	var cfBox = new conformBox();
	cfBox.show(checkDeleteFrom, ok, cancel);
	cfBox.accept = function() {
		var url = "LiveTVMain?CMD=2";
		url += "&subjId=" + temIdmenuGroup;
		url += "&t=" + Math.random();
...
function showfromUpdateMenu() {
	...
	for ( var i = 0; i < listArr.length; i++) {
		if (listArr[i].Id == temIdmenuGroup) {
			name = listArr[i].Name;
			image = listArr[i].UrlIma;
			break;
		}
	}
	var textbox = new newBox(subjectInfo, name, image, _titleName, _Image, ok,
			cancel);
	textbox.show();
	textbox.accept = function(text, image) {
		image = image.substring(image.lastIndexOf("/") + 1);
...
function changeTab(id) {
	...
	if (id == 0) {
		...
		$("div_subject").innerHTML = "Channel Advertise";
		$("id_table2").style.display = 'block';
		$("id_table").style.display = 'none';
		$("id_table1").style.display = 'none';
		$("menutree").style.display = 'none';
		$("list_channel").style.display = 'block';
		$("frmSearch").style.display = 'none';
		$("totalUser").style.display = 'none';
...
function loadChannelInfor(mId, cstatus) {
	if (checkTab == 0) {
		...
		if (checkSearch == 1 && cstatus != 1) {
			...
			// checkstatus = 1;
			// checkSearch = 1;
			// var textSearch = $("textSearch").value
			// page = 6;
			// var url = "LiveTVMain?CMD=11";
			// url += "&pageIndex=" + Index;
			// url += "&page=" + page;
			// url += "&nameSearch=" + textSearch;
			...
			// var _id =
			// it[i].getElementsByTagName("id")[0].childNodes[0].nodeValue;
			// var _stt =
			// it[i].getElementsByTagName("stt")[0].childNodes[0].nodeValue;
			// var _channelCode = it[i]
			// .getElementsByTagName("ChannelCode")[0].childNodes[0].nodeValue;
			// var _addr =
			// it[i].getElementsByTagName("address")[0].childNodes[0].nodeValue;
			// var _urlIma =
			// it[i].getElementsByTagName("urlIma")[0].childNodes[0].nodeValue;
			// var _serviceName = it[i]
			// .getElementsByTagName("serviceName")[0].childNodes[0].nodeValue;
			// var _active =
			// it[i].getElementsByTagName("Active")[0].childNodes[0].nodeValue;
			// var _sl =
			// it[i].getElementsByTagName("sl")[0].childNodes[0].nodeValue;
			// arr[i] = new ItemChannel(_name, _id, _stt, _channelCode,
			// _addr, _urlIma, _serviceName, _active, _sl);
			// if (_id == tempId) {
			// Status = _active;
			// }
			// }
			// document.getElementById("id_table").innerHTML = getHTMLUser(
			// arr, count);
			...
			var f = new AjaxGetXML(url);
			f.complet = function(tx) {
				// // refresh page se kiem soat tinh trang die service 24.1
				// if (tx == null) {
				// // var albox = new alertBox();
				...
				// var _sl =
				// it[i].getElementsByTagName("sl")[0].childNodes[0].nodeValue;
				// arr[i] = new ItemChannel(_name, _id, _stt,
				// _channelCode, _addr, _urlIma, _serviceName,
				// _active, _sl);
				// if (_id == tempId) {
				// Status = _active;
				// }
	...
	/*
	 * else if (checkTab == 2) { tempIdMenu = mId; var nameSubSer = ""; for (
	 * var i = 0; listArr.length; i++) { if (listArr[i].Id == mId) { nameSubSer =
	 * listArr[i].Name; break; } } $("namSerSub").innerHTML = nameSubSer;
	 * OnloadChannel(mId); OnloadLive(mId); ClickItem(mId); }
	 */
}
function search() {
	if (checkSearch == 0) {
...
function getHTMLUser(arr, count) {
	...
	html += "</div>";
	/*
	 * if(count ==0){ if(aa!=0){ html+="<div id='addusergroup1' align='center'
	 * style='display:block'>"; html+="<a href='javascript:showfromAddUser()'
	 * class='div_addeImage' title='Click here add User'>"+ AddUser +"</a>"
	 * html+="</div>"; } }
	 */
...
this.fucplaylink = function(rel) {
	pageclick();
	/*
	 * var url="LiveTVMain?CMD=100"; url+="&subjctId="+tempIdMenu;
...
function contextTableEX() {
	...
	this.draw = function(x, y) {
		...
		html += "<a class='playlink' rel='" + rellink + "'>" + checkLinkLiveTV
				+ "</a>";
		html += "</li>";

		// ///////////// change channelcode channel ////////////////////////
		html += "<li class='icon_contextmenuEdit'>";
		html += "<a>Change Order</a>";
		html += "</li>";
	...
	this.renderClick = function() {
		...
		var changeorder = li[4].getElementsByTagName("a")[0];
...
function ShowHTMLMenu(arr) {
	...
	for ( var i = 0; i < arr.length; i++) {
		...
		html += "<a pos='l' onclick='closeMenu()' href='javascript:loadChannelInfor("
				+ arr[i].Id
				+ ",1)' class='treeviewleaves_src' id='"
				+ arr[i].Id + "'>";
		html += arr[i].Name;
		html += "</a>";
		html += "</li>";
...
function contextMenu() {
	...
	this.draw = function(x, y) {
		...
		html += "</li>";
		html += "<li class='icon_contextmenuDel'>";
		html += "<a>" + deleteSubject + "</a>";
		html += "</li>";
		html += "<li class='icon_contextmenuEdit'>";
		html += "<a>" + subjectInfo + "</a>";
		html += "</li>";
		html += "</ul>";
		if ($("div_contextMenu") == undefined) {
			var div_ = document.createElement("div");
	...
	this.fucdel = function() {
		deleteMenuSubject(temIdmenuGroup);
		closeMenu();
	};
...
function OnloadChannel(_tempIdMenu) {
	var c = 1;
	var url = "LiveTVMain?CMD=8";
	url += "&subjectId=" + _tempIdMenu;
	url += "&r=" + Math.random();
	var f = new AjaxGetXML(url);
	f.complet = function(tx) {
		arrChannel = new Array();
		var xml = tx.getElementsByTagName("xml");
		BDcount = xml[0].getAttribute("count");
		var it = tx.getElementsByTagName("Item");
		for ( var i = 0; i < it.length; i++) {
			var _id = it[i].getElementsByTagName("id")[0].childNodes[0].nodeValue;
			var _name = it[i].getElementsByTagName("serviceName")[0].childNodes[0].nodeValue;
			var _active = it[i].getElementsByTagName("active")[0].childNodes[0].nodeValue;
			arrChannel[i] = new ItemTVChannel(_id, c++, "", _name, _active);
			/*
			 * if(_id==tempId){ StatusLive =_active; }
			 */
		}
		tableSrv.show(arrChannel);
		document.getElementById("totalUser").innerHTML = ShowTotalUser(BDcount);
	};
}
function OnloadLive(_tempIdMenu) {

	var c = 1;
	var url = "LiveTVMain?CMD=7";
	url += "&subjectId=" + _tempIdMenu;
	url += "&r=" + Math.random();
	var f = new AjaxGetXML(url);
	f.complet = function(tx) {
		...
		for ( var i = 0; i < it.length; i++) {
			...
			var _name = it[i].getElementsByTagName("name")[0].childNodes[0].nodeValue;
			var _active = it[i].getElementsByTagName("active")[0].childNodes[0].nodeValue;
			arrLive[i] = new ItemTVChannel(_id, c++, _name, "", _active);
			/*
			 * if(_id==tempId){ StatusLive =_active; }
			 */
		}
		tableCtn.show(arrLive);
	};
}
...
function AddChannels() {
	...
	this.onclickFile = function(name) {
		var ar = new Array(".mov", ".mkv", ".mp4", ".avi", ".mp3");
...
function showFormChangeOrder() {
	pageclick();
	var layer = new layer_vitual();
	layer.show();
	var url = "../ServiceJSP?ID=11";
	url += "&t=" + Math.random();
	var f = new AjaxGetText(url);
	f.complet = function(tx) {
		layer.addHtml(tx);
		setCenterDIV("form_change_order");
		initTinymce(2, "540px");
		$("changeok").onclick = function() {
			my.accept();
		};
		$("changecancel").onclick = my.cancel;
		getChannelcode();
		$("listtemp").value = "";
	};
	this.accept = function() {
		var text = $("listtemp").value;
		if(text == ""){
			my.hide();
			my.update();
			return;
...
//		}
		...
		f.complet = function(tx) {
			if(tx == "true"){
				...
				var albox = new alertBox();
				albox.show("Change channel code error???");
				return;
			}
...
function getChannelcode(){
	...
	f.complet = function(tx) {
		...
		for ( var i = 0; i < it.length; i++) {
			...
			var item = new ItemCode(_id, _name, _code);
			arr[i] = item;
		}
		document.getElementById("div_list_code").innerHTML = '';
		document.getElementById("div_list_code").innerHTML = ShowHTMLListCode(arr);
	};
}
function ItemCode(id, name, code){
	this.Id = id;
	this.Name = name;
	this.Code= code;
}
//var listcode = new Array();
function ShowHTMLListCode(arr) {
	listcode = [];
	var html = "";
	var m = arr.length % 4;
	var len = arr.length - m;
	for ( var i = 0; i < len; i = i + 4) {
		html += "<div class='pms_textbox_input' style='height: auto; margin-left: 20px; margin-top: 10px;'>";
		html += "<input type='text' size='26' style='border: 1px solid #dddddd; float: left; width: 20px;' id='c"+arr[i].Id+"' value="+arr[i].Code+" onkeypress='validate(event);' onkeyup='changeCode(\"c"+arr[i].Id+"\");' maxlength='3'/>";
		html += "<div class='pms_name_input' style='width: 270px;'>"+arr[i].Name+"</div>";
		
		html += "<input type='text' size='26' style='border: 1px solid #dddddd; float: left; width: 20px;' id='c"+arr[i+1].Id+"' value="+arr[i+1].Code+" onkeypress='validate(event);' onkeyup=changeCode(\"c"+arr[i+1].Id+"\") maxlength='3'/>";
		html += "<div class='pms_name_input' style='width: 270px;'>"+arr[i+1].Name+"</div>";
		
		html += "<input type='text' size='26' style='border: 1px solid #dddddd; float: left; width: 20px;' id='c"+arr[i+2].Id+"' value="+arr[i+2].Code+" onkeypress='validate(event);' onkeyup='changeCode(\"c"+arr[i+2].Id+"\");' maxlength='3'/>";
		html += "<div class='pms_name_input' style='width: 270px;'>"+arr[i+2].Name+"</div>";
		
		html += "<input type='text' size='26' style='border: 1px solid #dddddd; float: left; width: 20px;' id='c"+arr[i+3].Id+"' value="+arr[i+3].Code+" onkeypress='validate(event);' onkeyup='changeCode(\"c"+arr[i+3].Id+"\");' maxlength='3'/>";
		html += "<div class='pms_name_input' style='width: 270px;'>"+arr[i+3].Name+"</div>";
		html += "</div>";
//		listcode[i]=arr[i].Code;
//		listcode[i+1]=arr[i+1].Code;
//		listcode[i+2]=arr[i+2].Code;
//		listcode[i+3]=arr[i+3].Code;
	}
	if(m > 0){
		html += "<div class='pms_textbox_input' style='height: auto; margin-left: 20px; margin-top: 10px; margin-bottom: 10px'>";
		for ( var j = 0; j < m; j++) {
			html += "<input type='text' size='26' style='border: 1px solid #dddddd; float: left; width: 20px;' id='c"+arr[len + j].Id+"' value="+arr[len + j].Code+" onkeypress='validate(event);' onkeyup='changeCode(\"c"+arr[len + j].Id+"\");' maxlength='3' />";
			html += "<div class='pms_name_input' style='width: 270px;'>"+arr[len + j].Name+"</div>";
//			listcode[len + j]=arr[len + j].Code;
		}
		html += "</div>";
	}else
...
function changeCode(id) {
	var len = listcode.length;
//	var flag = false;
	if (len > 0) {
		for ( var i = 0; i < len; i++) {
			if(listcode[i].indexOf(id) >= 0){
//				flag = true;
				listcode[i] = id + "," + $(id).value;
				takeCode(listcode);
				return;
			}
		}
	}
	listcode[len] = id + "," + $(id).value;
	takeCode(listcode);
}

function takeCode(listcode){
	var text = listcode[0];
	for ( var i = 1; i < listcode.length; i++) {
		text += "," + listcode[i];
	}
	$("listtemp").value = "";
	$("listtemp").value = text;
