function layer_vitual() {
	...
	this.show = function() {
		if ($("div_layer_vitual") == undefined) {
			...
			$("div_layer_vitual").style.height = document.body.clientHeight
...
function conformBox() {
	...
	this.show = function(text, ok, cancel) {
		...
		html += "<input type=\"button\" value='" + cancel
				+ "'   class='div_buton' id=\"button_cancel\">";
		html += "</div>";
		html += "</div>";
		html += "</div>";
		layer.addHtml(html);
...
function BoxSubject(title, value, image1, image2) {
	...
	this.show = function() {
		...
		var html = "";
		var linkImg1 = linksaveimage + image1;
		var linkImg2 = linksaveimage + image2;
		html += "<div class=\"form_textbox\" id=\"form_textbox\" style='width: 440px;height:370;'  align='center'>";
		html += "<div class=\"form_textbox_subject\"><div class='div_title_form_new_src' >"
				+ title + "</div></div>";
		...
		if (image1.length > 0) {
			html += "<img id='image1' src=\"" + linkImg1
		...
		if (image2.length > 0) {
			html += "<img id='image2' src=\"" + linkImg2
					+ "\" width='170' height='150'></img>";
		} else {
...
function newBox(title, name, image, titleName, titleIma, ok, cancel) {
	...
	this.show = function() {
		...
		var html = "";
		var linkImg = linksaveimage + image;
		html += "<div class=\"form_textbox\" id=\"form_textbox\"  align='center'>";
		html += "<div class=\"form_textbox_subject\"><div class='div_title_form_new_src' style='background-color:#D3D0BE'>"
				+ title + "</div></div>";
		...
		html += "<input type=\"text\"    style='height:24;width:185px;float:left;'  id=\"form_textbox_input\" value='"
				+ name + "'/>";
		html += "</div>";
		html += "<div class=\"form_textbox_input\">";
		html += "<div class='div_image_input'  >" + titleIma + ":</div>";
		html += "<div  style='float:left;width:180px;height: 25px;margin-top: 8px' >";
		html += "	<iframe class='frame_upload' src=\"../upload\" style='overflow-x: hidden;overflow-y: hidden;overflow: hidden;' valign=\"top\" scrolling=\"no\" frameborder='0' width='180px' height=\"40px\" ></iframe>";
		html += "</div>";
		...
		html += "<input type=\"button\" style='margin-left: 15px;' value='"
				+ ok + "'  class='div_buton'id=\"form_textbox_ok\" >";
		html += "<input type=\"button\" value='" + cancel
...
function newBox1(title, name, serviceName, image, titleSName, titleName,
		titleIma, ok, cancel, url, address_) {
	var my = this;
	this.show = function() {
		if ($("div_layer_vitual") == undefined) {
			var div = document.createElement("div");
		...
		html += "<div class='form_textbox_center'>";
		html += "<div class='left_textbox'>";
		html += "<div class=\"form_textbox_input\" style='margin-top: 10px;'>";
		html += "<div class='div_name_input'>" + titleName + ":</div>";
		html += "<input type=\"text\"   style='border: 1px solid #dddddd;height:24;width:185px;float:left;'  id=\"form_textbox_input\" value='"
				+ serviceName + "'/>";
		html += "</div>";

		html += "<div class=\"form_textbox_input\" style='margin-top: 10px;'>";
		html += "<div class='div_name_input'>" + url + ":</div>";
		html += "<input type=\"text\"   style='border: 1px solid #dddddd;height:24;width:185px;float:left;'  id=\"url_address\" value='"
				+ address_ + "'/>";
		html += "</div>";

		html += "<div class=\"form_textbox_input\" style='margin-top: 5px;'>";
		html += "<div class='div_image_input'>" + titleIma + ":</div>";
		...
		html += "<div class='form_1'>";
		// loai bo nut ok vi khong can thiet vi ko the hieu chinh tung livetv
		// 18.1
		html += "<input type=\"button\" margin-left: 15px;' value='" + ok
				+ "'  class='div_buton'id=\"form_textbox_ok\" >";
		html += "<input type=\"button\" value='" + cancel
				+ "'  class='div_buton' id=\"form_textbox_cancel\">";
		html += "</div>";
		html += "</div>";
		...
		$("form_textbox_ok").onclick = function() {
			var value = $("form_textbox_input").value;
			var image = $("image").src;
			var link = $("url_address").value;
			my.accept(value, image, link);
		};
		$("form_textbox_cancel").onclick = my.cancel;
	};
