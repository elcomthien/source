...
function AddContent()
{
	...
	dropSubCtn.Select=function(obj,x)
	{						
		this.SubID=obj.Id;
		temp =obj.Id;
...
//		}
	...
	dropSubSrv.Select=function(obj,x)
	{
		...
		{
			$("id-subject-service").value = obj.Id;
			...
			this.SubID=obj.Id;
//			if(x == "two"){
//				tableCtn.load($("id-subject-content").value);
//			}
		}
	};
	this.init=function()
...
function TableCtn()
{
	...
	this.show=function()
	{			
		var html="<table id='table-vod-content-service' class='table2' border=\"0\" cellspacing=\"0\" cellpadding=\"0\">";
		html+="<thead class='fixedHeader'>";
		html+="<tr>";
				html+="<th style='padding-left:5px' align=\"left\" >";
				html+=lang.movieTitle;
		...
		html+="<tbody class='scrollContent'>";
			...
			html+="</tbody>";
			html+="</table>";	
//			$("table-vod-content-service").remove();
			$(divName).innerHTML=html;			
			this.renderEvent();
	};
...
//			url+="&SubId="+this.CtnId;
			url+="&SubId="+$("id-subject-content").value;
			url+="&subISvr="+tempsrc;
			url+="&r="+Math.random();			
			var f=new AjaxGetXML(url);			
			f.complet=function(tx)
...
function dropSubjectCtn()
{
	...
	{					
		...
		{			
			...
			if (x == "one") {
				$("id-subject-content").value = my.List[0].Id;
				my.Select(my.List[0],"one");
			}
			if(x == "two"){
				my.ClickItem("\n" + $("id-subject-content").value);
			}
//			my.Select(my.List[0],"one");
		};		
	};
...
function getListContent2Table(subCnt, subSrv){
//	alert(subCnt + " - " + subSrv);
}