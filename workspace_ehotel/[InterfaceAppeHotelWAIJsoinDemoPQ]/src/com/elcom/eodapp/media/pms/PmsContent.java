...
public class PmsContent {
	...
	public String getMessageInfo(int messId, String smartcard) {
		...
		xml = xml + "<sender>" + UnicodeConverter.encodeUnicode(aMessage.getSender()) + "</sender>\r\n";
		xml = xml + "<subject>" + UnicodeConverter.encodeUnicode(aMessage.getSubject()) + "</subject>\r\n";
	...
	public String getFolioMessages(String smartcard) {
		...
		for (int i = 0; i < eMessages.size(); i++) {
			...
			xml = xml + "<sender>" + UnicodeConverter.encodeUnicode(item.getSender()) + "</sender>\r\n";
			xml = xml + "<subject>" + UnicodeConverter.encodeUnicode(item.getSubject()) + "</subject>\r\n";
	...
	public String orderRoomService(eItemOrder item, String smartcard) {
		...
		param = new SubProParam(item.getDetail(), SubProParam.IN);
	...
	public static Vector<eItemDining> LoadItemDining(Vector<String> vTmp) {
		...
		// System.out.println(i + "---" + vTmp.get(i));
		...
		for (int i = 0; i < vTmp.size(); i = i + 13) {
			...
			int count = Integer.parseInt((String) vTmp.get(i + 12));
			if (count > 0) {
				for (int j = i + 13; j < i + 13 + count * 2; j += 2) {
					HashMap<String, String> map = new HashMap<String, String>();
					map.put("id", (String) vTmp.get(j));
					map.put("def", (String) vTmp.get(j + 1));
					list.add(map);
				}
				i = i + count * 2;
			}
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	public HashMap<String, String> getNotifyChannel(String key) throws IOException {
		...
		try {
			params = broker.executeSubPro(SQL.sqlgetNotifyChannel, params);
			if ((params != null) & (params.size() > 0)) {
				out_data = (SubProParam) params.get(1);
				outParam = out_data.getVector();
			}
		...
		obj.put("size", outParam.get(0));
		obj.put("color", outParam.get(1));
		obj.put("text", outParam.get(2));
		return obj;
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	public JSONArray filterFlight(String location, String station, String date, String route, String airline, String flighttype)
			throws IOException {
		JSONObject obj = new JSONObject();
		JSONArray ja = new JSONArray();
		Vector params = new Vector();
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println(p.getItemOfService("872", "509KCTB0R865", "-1", "-1"));
	}
}
