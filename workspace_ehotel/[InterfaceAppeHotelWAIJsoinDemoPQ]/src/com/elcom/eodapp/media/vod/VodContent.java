...
public class VodContent {
	...
	// ------------------------------------------------------------------------------
	public ArrayList exeBrowserData(String keystb, String whereStmt, Vector params, int fromRow, int noRows) {
		...
		Vector vRS = broker.executeSelect(sql.getSqlBrowser() + whereStmt, params, fromRow, noRows);
	...
	public String loadDataFilmXML(Vector vRS) throws IOException {
		...
		String Servicename, Productor, Director, Actors, Plot, Writer, Milisecond, Poster, Starnumber, Urlfilm, Price, Iunit, Backgound;

		VodCtnVO vo;

		for (int i = 0; i < vRS.size(); i = i + 16) {
			obj = new JSONObject();
			vo = new VodCtnVO(); // new an instance of MODCtnVO
			...
			Backgound = (String) vRS.get(i + 15);

//			/*
//			 * Productor = "nothing"; Director = "nothing"; Writer = "nothing"; Starnumber = "nothing";
//			 */
