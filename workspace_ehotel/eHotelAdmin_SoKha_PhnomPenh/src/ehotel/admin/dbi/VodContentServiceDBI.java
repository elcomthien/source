...
public class VodContentServiceDBI {
	...
	public List<Subject> getSubjectServiceCharge(int lang){
		...
		String sql = "select subjectid,subjectname,url_image,parent_id from vod_servicesubject where parent_id in (23,34) and SUBJECTID <> 34 and lang_id=2 order by SUBJECTID";
		Vector vector = new Vector();
		vector = broker.executeSelect(sql, null);
		if(vector.size()>2){
			ResultDB result = new ResultDB(vector);
