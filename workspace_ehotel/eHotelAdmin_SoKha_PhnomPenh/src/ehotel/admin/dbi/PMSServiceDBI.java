...
//			System.out.println(mess);
	...
	@SuppressWarnings("unchecked")
	public int updateInvisibleDiningMenu(int id) {
		int rs = -1;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(id), 0);
		params.add(in);
		...
		params.add(out);
		try {
			params = broker.executeSubPro(UPDATE_DININGMENU_INVISIBLE, params);
			if ((params != null) & (params.size() > 0)) {
				SubProParam paramOUT = (SubProParam) params.get(1);
				rs = Integer.parseInt(paramOUT.getString().trim());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(p.getWelcomeMessage());
	}
}
