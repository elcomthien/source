...
public class ServiceActiviti extends ServiceParent {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case 4://show form detail
		{
			...
			this.showJSPpage(request, response, "/pmsMng/hotel/detailActiviti.jsp");	
			break;
		}
		...
		case 1://insert pms subject
		{
			...
			{								
				...
				if(id>0)
				{
					...
					String path2=config._pathImage+config._activities + "/" +image;
					file.copy(path1, path2);
					file.deletefile(path1);
					path1=config._temp+"/"+urlBg;
					path2=config._pathImage+config._activities + "/" +urlBg;
					file.copy(path1, path2);
					file.deletefile(path1);
				}	
				out.write(id);
			...
			if(request.getParameter("name")!=null)
			{
				...
				{			
					...
					{						
						...
						path2=config._pathImage+config._activities + "/" +bgimage;
						file.copy(path1, path2);
						file.deletefile(path1);
					}
				}
