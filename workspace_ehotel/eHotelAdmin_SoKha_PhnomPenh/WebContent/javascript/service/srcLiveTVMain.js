...
function getHTMLUser(arr, count) {
	...
	html += No;
	html += "</th>";
	html += "<th style='margin-left: 20px;' align='left'height='30' width=\"" + 30
			+ "%\" valign=\"middle\"  >";
	html += name;
	html += "</th>";
...
function AddChannels(){
	...
	this.onclickFile = function(name) {
		var ar = new Array(".mov", ".mkv", ".mp4", ".avi", ".wmv");
		...
		if (!t) {
			var albox = new alertBox();
			albox.show("Please select file .mov .mkv .mp4 .avi .wmv!");
		} else {
...
function deleteChannel(channelID, channelurl) {
	closeMenu(0);
	var cfBoxc = new conformBox();
	cfBoxc.show(checkDeleteFrom, ok, cancel);
	cfBoxc.accept = function() {
		var wt = new Waiting();
		wt.show(null);
...
function showfromUpdateChannel() {
	...
	var textbox = new newBox("Info Channel", name, image, _titleName, _Image, ok, cancel);
	textbox.show();
	textbox.accept = function(text, image) {
		image = image.substring(image.lastIndexOf("/") + 1);
		if (text.length <= 0) {
		...
		for(var j = 0; j < listChannel.length; j++){
			if(listChannel[j].Name == text && listChannel[j].Id != temIdmenuGroup){
				var albox = new alertBox();
				albox.show(checkEditEx);
				return;
			}
