...
ctMenu.fucNew = function(idP) {
	...
	html += "<input type=\"button\" value='" + ok
			+ "' id=\"form_textbox_okss\" class='div_buton' >";
	...
	f.complet = function(tx) {
		...
		for ( var i = 0; i < it.length; i++) {
			...
			if (tempCategoryId == _id) {
				...
				html += "<div style='width:50%;float:left'><input type=\"text\" size='38' id='txtUpCategory' class='textbox_input' value='"
						+ NameCate + "'  /></div>";
				html += "</div>";
				html += "<div  style='float: left;width: 100%;' align='center'>";
				html += "<div style='margin-left: 40px'  class='div_sub_buton'>";
				html += "<input type=\"button\" value='" + ok
						+ "' id=\"form_textbox_oks\" class='div_buton' >";
				html += "<input type=\"button\" value='" + cancel
						+ "' id=\"form_textbox_cancels\" class='div_buton'>";
				html += "</div>";
				html += "</div>";
				html += "</div>";
				layer.addHtml(html);
...
function acceptUpdateRole() {
	...
	/*
	 * if(select2.options.length<=0) { var albox=new alertBox();
	 * albox.show("please select item in select 2 !"); return; }
	 */
	var param = "";
...
function showfromRoleMng() {
	...
	if (isAdmin == 'true') {
		html += "<input type=\"button\" 	' value='" + update
				+ "' id=\"form_textbox_ok\"/ style='margin-right:25px' >"
	}
	html += "</div>"
	html += "</div>"
	html += "</div>"
...
this.loadleft = function(id) {
	...
	f.complet = function(tx) {
		...
		var html = ""
		html += "<div align='left' style='color:#ab8718'>" + NameNotAssigned
				+ "</div>";
...
this.loadright = function(id) {
	...
	f.complet = function(tx) {
		...
		var html = ""
		html += "<div align='left'  style='color:#ab8718;margin-left:5px'>"
				+ NameAssigned + "</div>";
