...
var videoreport = new function ReportVideo() {
	var my = this;
...
function Monthly() {
	...
	this.loadtime = function() {
		...
		/*
		 * html+="<option>2013</option>"; html+="<option>2014</option>";
		 * html+="<option>2015</option>"; html+="<option>2016</option>";
		 * html+="<option>2017</option>"; html+="<option>2018</option>";
		 * html+="<option>2019</option>"; html+="<option>2020</option>";
		 * html+="<option>2021</option>"; html+="<option>2022</option>";
		 */
		html += "</select>";
		html += "</div>";
		/*
		 * html+="<div class='bt_charst' title=\"charts\" id='bt_charst'>";
		 * html+="</div>"; html+="<a class='bt_excel' title=\"export excel\"
		 * id='bt_excel'>"; html+="</a>";
		 */
	...
	this.showcharst = function() {
		...
		html += "<iframe src=\"../charts.jsp?year="
				+ my.year
				+ "\" scrolling=\"no\" frameborder='0' width=\"98%\" height=\"400px\" ></iframe>";
		html += "</div>";
		html += "</div>";
		layer.addHtml(html);
...
function StaticGenric() {
	...
	this.loadtime = function() {
		...
		/*
		 * html+="<div style=\"float: left;width: 100px;\">";
		 * html+=langpms.from_date; html+="</div>"; html+="<div style=\"float:
		 * left;width:130px;\">"; html+="<input size=\"10\" id=\"f_date1\"/><button
		 * id=\"f_btn1\" class='buttom_calenda'>...</button>"; html+="</div>";
		 * html+="<div style=\"float: left;width: 100px;\">";
		 * html+=langpms.to_date; html+="</div>"; html+="<div style=\"float:
		 * left;width: 130px;\">"; html+="<input size=\"10\" id=\"f_date2\" /><button
		 * id=\"f_btn2\" class='buttom_calenda'>...</button>"; html+="</div>";
		 */
		html += "<div style=\"float: left;width: 100px;\">";
		html += langpms.select_year;
		html += "</div>";
		...
		/*
		 * var now = new Date(); var prev = new Date();
		 * prev.setMonth(prev.getMonth() - 1); var time2=
		 * now.getDate()+"/"+(now.getMonth()+1)+"/"+now.getFullYear();
		 * $("f_date2").value=time2; var time1=
		 * prev.getDate()+"/"+(prev.getMonth()+1)+"/"+prev.getFullYear();
		 * $("f_date1").value=time1; var cal1= Calendar.setup({ inputField :
		 * "f_date1", trigger : "f_btn1", onSelect : my.changeDatefrom,
		 * dateFormat : "%d/%m/%Y", date:prev }); var cal2= Calendar.setup({
		 * inputField : "f_date2", trigger : "f_btn2", onSelect :
		 * my.changeDateNow, dateFormat : "%d/%m/%Y", date: now });
		 */
		my.get();
	...
	this.get = function() {
		my.List.length = 0;
		/*
		 * var from =$("f_date1").value; var to =$("f_date2").value; var
		 * url="VideoReport?CMD=1"; url+="&pageIndex="+Index;
		 * url+="&from="+from; url+="&to="+to; url+="&r="+Math.random(); var
		 * f=new AjaxGetXML(url); f.complet=function(tx) { var arr=new Array();
		 * var xml=tx.getElementsByTagName("xml"); var
		 * it=tx.getElementsByTagName("Item"); for(var i=0;i<it.length;i++) {
		 * var
		 * _name=it[i].getElementsByTagName("name")[0].childNodes[0].nodeValue;
		 * var _id=it[i].getElementsByTagName("id")[0].childNodes[0].nodeValue;
		 * var
		 * _Amount=it[i].getElementsByTagName("Amount")[0].childNodes[0].nodeValue;
		 * arr[i]=new itemData(_id,_name,_Amount); } my.List=arr;
		 * table.load(my.List); }
		 */
		var url = "VideoReport?CMD=2";
		url += "&pageIndex=" + Index;
		url += "&month=" + my.year;
		url += "&r=" + Math.random();
		var f = new AjaxGetXML(url);
...
function StaticFilm() {
	...
	this.loadtime = function() {
		...
		/*
		 * html+="<a class='bt_excel' title=\"export excel\" id='bt_excel'>";
		 * html+="</a>"; $("div_time").innerHTML=html;
		 * $("bt_excel").onclick=my.excel; var now = new Date(); var prev = new
		 * Date(); prev.setMonth(prev.getMonth() - 1); var time2=
		 * now.getDate()+"/"+(now.getMonth()+1)+"/"+now.getFullYear();
		 * $("f_date2").value=time2; var time1=
		 * prev.getDate()+"/"+(prev.getMonth()+1)+"/"+prev.getFullYear();
		 * $("f_date1").value=time1; var cal1= Calendar.setup({ inputField :
	...
	table.dataBind = function() {
		...
		for ( var i = 0; i < this.List.length; i++) {
			...
			/*
			 * html+="<td align='center'  valign=\"middle\">";
			 * html+=this.List[i].Amount; html+="</td>";
			 */
			html += "</tr>";
		}
		html += "</table>";
	...
	this.get = function() {
		my.List.length = 0;
		/*
		 * var from =$("f_date1").value; var to =$("f_date2").value;
		 */
		var url = "VideoReport?CMD=3";
		url += "&pageIndex=" + Index;
