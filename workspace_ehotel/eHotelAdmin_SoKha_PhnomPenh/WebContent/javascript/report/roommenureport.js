var svcmenurpt=new function RoomServiceMenuReport()
{
	var my=this;
	var table=new Table();
//	var Index=0;
	...
	table.dataBind=function()
 	{
		...
		html += "No";
		html += "</th>";
		html += "<th align='left'  style='text-align: left;'  width=\"" + 30 + "%\"  >";
		html += "Food Items";
		html += "</th>";
		html += "<th align='center' width=\"" + 20 + "%\" style='text-align: center;'  >";
		html += "Updated Date";
		html += "</th>";
		html += "<th align='center' width=\"" + 15 + "%\" style='text-align: center;'  >";
		html += "Rate";
		html += "</th>";
		html += "<th align='center' width=\"" + 15 + "%\" style='text-align: center;'  >";
		html += "Currency";
	...
	this.get = function() {
		...
		var url = "ServiceMenuReport?CMD=1";
		url += "&from=" + from;
		url += "&to=" + to;
		url += "&r=" + Math.random();
...
//			var xml = tx.getElementsByTagName("xml");
			...
			for ( var i = 0; i < it.length; i++) {
				...
				var _updatedate = it[i].getElementsByTagName("updatedate")[0].childNodes[0].nodeValue;
				...
				var _updateby = it[i].getElementsByTagName("updateby")[0].childNodes[0].nodeValue;
				arr[i] = new itemData(_name, _updatedate, _rate, _currency,_updateby);
			}
			my.List = arr;
			table.load(my.List);
		};
	};
	function itemData(name,time,rate,currency,updateby)
	{
		this.Name = name;
		this.Time = time;
		this.Rate = rate;
