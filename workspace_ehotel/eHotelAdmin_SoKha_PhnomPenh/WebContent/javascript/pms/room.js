...
//		content = encode(content);
	...
	ctTable.fuceditcharge = function() {
		...
		f.complet = function(tx) {
			...
			initTinymce(2, "540px");
			$("old_charge").value=me.obj.Charge;
...
//				var _roomcode = it[i].getElementsByTagName("status")[0].childNodes[0].nodeValue;
				...
				arr[i] = new itemFolio(_id, _name, _type, _key, _status,_charge, _fee, _xxx, _free);
			}
			table.load(arr);
...
function contextTableEX() {
	...
	this.renderClick = function() {
		...
		var fuceditcharge = li[8].getElementsByTagName("a")[0];
		fuceditcharge.onclick = function() {
			my.fuceditcharge();
		};
		
		var fucupdateota = li[9].getElementsByTagName("a")[0];
...
function itemFolio(id, name, type, key, status, charge, fee, xxx, free) {
	this.Id = id;
	this.Name = name;
	this.Key = key;
	this.Status = status;
