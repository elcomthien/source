...
function loadUserInfor(mId) {
	...
	f.complet = function(tx) {
		...
		for ( var i = 0; i < it.length; i++) {
			...
			arr[i] = new ItemUrlUser(_stt, _id, _name, _department,
					_createDate, _Active, _addr, _account);
			if (_id == tempId) {
				Status = _Active;
			}
		}
...
/*
 * this.oncontextmenu=function(id,e) { var obj=null; for(var i=0;i<arr.length;i++) {
 * if(arr[i].Id==id) { Status =arr[i].Active; tempId =id; obj=arr[i]; break; } }
 * ctTable.load(obj, e); }
 */
/*
 * this.oncontextmenu1=function(id,e) { var obj=null; for(var i=0;i<arr.length;i++) {
 * if(arrGroup[i].Id==id) { obj=arrGroup[i]; break; } } ctMenu.load(obj, e); }
 */
...
function ShowHTMLMenu(arr) {
	...
	for ( var i = 0; i < arr.length; i++) {
		...
		html += "<a pos='l' onclick='closeMenu()' href='javascript:loadUserInfor("
				+ arr[i].Id
				+ ")' class='treeviewleaves_admin' id='"
				+ arr[i].Id + "'>";
		html += arr[i].Name;
		html += "</a>";
		html += "</li>";
...
function getHTMLUser(arr, count) {
	...
	html += "<th align='left' style='padding-left:5px'  height='30' valign=\"middle\"  width=\""
			+ 10 + "%\"  >";
	html += no;
	html += "</th>";
	html += "<th align='left'height='30' width=\"" + 30
			+ "%\" valign=\"middle\"  >";
	html += name;
	html += "</th>";
	html += "<th align='left'height='30' width=\"" + 20
			+ "%\" valign=\"middle\"  >";
	html += department;
	html += "</th>";
	html += "<th align='left' height='30' width=\"" + 20
	...
	for (i = 0; i < arr.length; i++) {
		if (i % 2 == 0) {
			html += "<tr onclick='closeMenuTable(" + arr[i].Id + ")' class=\""
					+ bgRow2 + "\" id=\"" + arr[i].Id + "\"  >";
		} else {
			html += "<tr onclick='closeMenuTable(" + arr[i].Id + ")' class=\""
					+ bgRow1 + "\" id=\"" + arr[i].Id + "\"   >";
		}
		html += "<td height='30' style='padding-left:5px' width='10%'>"
				+ arr[i].Stt + "</td>";
		html += "<td height='30' width='30%' style='color: #9aa685;font-size:18'>"
				+ arr[i].Name + "</td>";
		html += "<td height='30' width='20%'>" + arr[i].Department + "</td>";
		html += "<td height='30' width='25%'>"
				+ (arr[i].CreateDate).substring(0, 11) + "</td>";
		if (arr[i].Active == 1) {
	...
	if (count == 0) {
		if (aa != 0) {
			...
			if (isAdmin == 'true') {
				html += "<a href='javascript:showfromAddUser()' class='div_addeImage' title='Click here add User'>"
						+ AddUser + "</a>";
			}
			html += "</div>";
		}
...
function showfromAddUser() {
	...
	html += "<input type=\"button\" value='"
			+ ok
			+ "' id=\"form_textbox_ok\" class='div_buton' style='margin-left: 155px;margin-top:5px'>";
	html += "<input type=\"button\" value='"
			+ cancel
			+ "' id=\"form_textbox_cancel\" class='div_buton' style='margin-top:5px'>";
	html += "</div>";
	html += "</div>";
	html += "</div>";
	html += "</div>";
...
function showfromUpdatePassUser() {
	...
	html += "<div style='height:20;width:420px;margin-bottom: 15px;margin-top: 5px;margin-left: 10px'>";
	html += "<div style='width:35%;float:left'>" + name
			+ "(<font color='red'>*</font>):</div>";
	html += "<div style='width:60%;float:left'>" + _name + "</div>";
	html += "</div>";
	html += "<div style='height:20;width:420px;margin-bottom: 15px;margin-top: 5px;margin-left: 10px'>";
	html += "<div style='width:35%;float:left'>" + password
			+ "(<font color='red'>*</font>):</div>";
	html += "<div style='width:60%;float:left'><input type=\"password\" size='38' id='txtPassAdd' class='textbox_input' style='width:100%;' /></div>";
	html += "</div>";
	html += "<div style='height:20;width:420px;margin-bottom: 15px;margin-top: 5px;margin-left: 10px'>";
	html += "<div style='width:35%;float:left'>" + cfPassword
			+ "(<font color='red'>*</font>):</div>";
	html += " <div style='width:60%;float:left'><input type=\"password\" size='38' id='txtcfPassAdd' class='textbox_input' style='width:100%;'/></div>";
	html += "</div>";
	...
	html += "<input type=\"button\" value='"
			+ ok
			+ "' id=\"form_textbox_ok\" class='div_buton' style='margin-left: 155px'>";
...
function showfromAddMenu() {
	...
	html += "<div style='width:20%;float:left'>" + name
			+ "(<font color='red'>*</font>):</div>";
	...
	f.complet = function(tx) {
		...
		for ( var i = 0; i < it.length; i++) {
			...
			if (_id == temIdmenuGroup) {
				...
				var html = "";
				html += "<div class=\"form_add_menu_sysAdmin\" id=\"form_detail_song\">";
				html += "<div class='div_Title_song' align=\"left\">"
						+ checkUpdateMenuEx + "";
				html += "</div>";
				html += "<div style='float: left;width:375px;height:70px ;border: 2px solid #dddddd;margin-left: 10px;margin-top: 10'>";
				html += "<div style='height:20;width:350px;margin-bottom: 15px;margin-top: 5px;margin-left: 10px'>";
				html += "<div style='width:20%;float:left'>" + name
						+ "(<font color='red'>*</font>):</div>";
				html += "<div style='width:50%;float:left'><input type=\"text\" size='38' id='txtUpMenu' class='textbox_input' value='"
						+ name1 + "'/></div>";
				html += "</div>";
				html += "<div  style='float: left;width: 100%;' align='center'>";
				html += "<div style='margin-left: 40px'  class='div_sub_buton'>";
...
function showfromDetailUser() {
	...
	html += "</div>";
	html += "<div style='height:20;width:350px;margin-bottom: 15px;margin-left:10px'>";
	html += "<div style='width:25%;float:left'>" + name
			+ "(<font color='red'>*</font>):</div>";
	html += "<div style='width:75%;float:left'> <input type=\"text\" size='38' id='txtNameUp' class='textbox_input' value='"
			+ _name + "' style='width:100%;'/></div>";
	html += "</div>";
	html += "<div style='height:20;width:350px;margin-bottom: 15px;margin-left:10px'>";
	html += "<div style='width:25%;float:left'>" + address + ":</div>";
	html += "<div style='width:75%;float:left'> <input type=\"text\" size='38' id='txtAddressUp' class='textbox_input' value='"
			+ _addr + "' style='width:100%;'/></div>";
	html += "</div>";
	html += "<div style='height:20;width:350px;margin-bottom: 15px;margin-left:10px'>";
	html += "<div style='width:25%;float:left'>" + department + ":</div>";
	html += " <div style='width:75%;float:left'><input type=\"text\" size='38' id='txtDepartmentUp' class='textbox_input' value='"
			+ _department + "' style='width:100%;'/></div>";
	html += "</div>";
	html += "<div  style='float: left;width: 100%;' align='center'>";
	...
	if (isAdmin == 'true') {
		html += "<input type=\"button\" value='"
				+ ok
				+ "' id=\"form_textbox_ok\" class='div_buton' style='margin-top: 10px' >";
	}
	html += "<input type=\"button\" value='"
			+ cancel
			+ "' id=\"form_textbox_cancel\" class='div_buton' style='margin-top: 10px'>";
	html += "</div>";
	html += "</div>";
	html += "</div>";
	html += "</div>";
...
this.showfromAddPmsGroup = function() {
	...
	html += "<div class='div_Title_song_admin' style='width:500px' >"
			+ NameGrant;
	html += "</div>";
	html += "<div style='margin-top:10px;margin-left:10px' >" + userNameGroup
			+ " : " + _name;
	html += "</div>";
...
this.loadleft = function() {
	...
	f.complet = function(tx) {
		...
		for ( var i = 0; i < arrPmsL.length; i++) {
			html += "<option value='" + arrPmsL[i].Id + "'>" + arrPmsL[i].Name
					+ "</option>";
		}
		html += "</select>";
...
this.loadright = function() {
	...
	f.complet = function(tx) {
		...
		for ( var i = 0; i < arrPmsR.length; i++) {
			html += "<option value='" + arrPmsR[i].Id + "'>" + arrPmsR[i].Name
					+ "</option>";
		}
		html += "</select>";
...
this.editUser = function(acc) {
	...
	url += "&tempId=" + tempId;
	url += "&t=" + Math.random();
	var param = "&name=" + name + "&address=" + address + "&department="
			+ department + "&account=" + acc;
...
function contextTableEX() {
	...
	this.draw = function(x, y) {
		...
		html += "</li>";
		/*
		 * if(Status==1) { html+="<li class='icon_visibility2'>"; }else {
		 * html+="<li class='icon_visibility1'>"; } html+="<a>"+changeStatus+"</a>";
		 * html+="</li>";
		 */
		html += "<li class='icon_contextmenuEdit'>";
	...
	this.renderClick = function() {
		...
		addpmsingroup.onclick = function() {
			showfromAddPmsGroup();
		};
...
this.loadleftChangeGroup = function() {
	...
	f.complet = function(tx) {
		...
		for ( var i = 0; i < arrPmsCGL.length; i++) {
			html += "<option value='" + arrPmsCGL[i].Id + "'>"
					+ arrPmsCGL[i].Name + "</option>";
		}
		html += "</select>";
		$("left_change_subject").innerHTML = html
		return arrPmsCGL;
...
this.loadrightChangeGroup = function() {
	...
	f.complet = function(tx) {
		...
		for ( var i = 0; i < arrPmsCGR.length; i++) {
			html += "<option value='" + arrPmsCGR[i].Id + "'>"
					+ arrPmsCGR[i].Name + "</option>";
		}
		html += "</select>";
	...
	this.loadleftGroup = function() {
		...
		f.complet = function(tx) {
			...
			for ( var i = 0; i < arrPmsGL.length; i++) {
				html += "<option value='" + arrPmsGL[i].Id + "'>"
						+ arrPmsGL[i].Name + "</option>";
			}
			html += "</select>";
			$("left_change_subject").innerHTML = html
			return arrPmsGL;
	...
	this.loadrightGroup = function() {
		...
		f.complet = function(tx) {
			...
			for ( var i = 0; i < arrPmsGR.length; i++) {
				html += "<option value='" + arrPmsGR[i].Id + "'>"
						+ arrPmsGR[i].Name + "</option>";
			}
			html += "</select>";
			$("right_change_subject").innerHTML = html;
			return arrPmsGR;
