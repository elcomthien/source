...
function showfromEditChannel() {
	...
	var textbox = new newBox1(subjectInfoLive, name, serviceName, image,
			_serviceName, _titleName, _Image, ok, cancel);
	textbox.show();
	textbox.accept = function(text, image) {
		image = image.substring(image.lastIndexOf("/") + 1);
		if (text.length <= 0) {
...
function AddChannels() {
	...
	this.accept = function() {
		var id = $("div_channel_id").value;
		if(id == "-1"){
			var albox = new alertBox();
			albox.show("This channel can't update!");
			return;
		}
		var channelname = $("div_channel_name").value;
		for ( var j = 0; j < listChannel.length; j++) {
			if (listChannel[j].Name == channelname && listChannel[j].Id != id) {
				var albox = new alertBox();
				albox.show(checkEditEx);
				return;
			}
...
function loadChannel() {
	...
	f.complet = function(tx) {
		...
		document.getElementById("list_channel").innerHTML = ShowListChannel(listChannel);
//		renderEventChannel();
		loadInfoChannel(listChannel[0].Id, encodeURI(listChannel[0].Name),
				encodeURI(listChannel[0].Parent), encodeURI(listChannel[0].UrlIma), 1);
		ClickItemChannel(listChannel[0].Id);
	};
};
...
// href='javascript:loadInfoChannel(" + obj + ")'
function ShowListChannel(arr) {
	var obj = null;
	var html = "";
	html += "<div class='treeview'>";
	html += "<ul style='padding-left:5px; margin-left:5px;'>";
	for ( var i = 0; i < arr.length; i++) {
		obj = arr[i];
		html += "<li pos='l' id='" + arr[i].Id + "'>";
		html += "<a pos='l' onclick=loadInfoChannel(" + obj.Id + ",'"
				+ encodeURI(obj.Name) + "','" + encodeURI(obj.Parent) + "','"
				+ encodeURI(obj.UrlIma)
				+ "',0)  class='treeviewleaves_src' id='" + arr[i].Id
	...
	html += "</ul>";
	html += "</div>";
//	loadInfoChannel(arr[0].Id, encodeURI(arr[0].Name),
//			encodeURI(arr[0].Parent), encodeURI(arr[0].UrlIma), 1);
	return html;
};
function loadInfoChannel(id, name, link, image, flag) {
	name = decodeURI(name);
	link = decodeURI(link);
	image = decodeURI(image);
	$("div_channel_id").value = id;
	$("div_channel_name").value = name;
	$("image_channel").src = image;
	if (flag == "0")
		ClickItemChannel(id);
}
