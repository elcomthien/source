var bar = new function Minibar() {
//	var my = this;
	var list = new ListMinibar();
	this.run = function() {
		list.load();
	};
...
function ListMinibar() {
	...
	ctTable.fucdel = function() {
		...
		cfBox.accept = function() {
			var url = "SvcHouseKeeping?CMD=12";
			url += "&id=" + ob.Id;
			url += "&name=" + ob.Name;
			url += "&r=" + Math.random();
	...
	ctTable.fucStatus = function() {
		var url = "SvcHouseKeeping?CMD=13";
		url += "&id=" + this.obj.Id;
...
//			html += "</td>";
	...
	this.get = function(Index) {
		var url = "SvcHouseKeeping?CMD=10";
		url += "&pageIndex=" + Index;
		url += "&page=6";
		url += "&r=" + Math.random();
		...
		f.complet = function(tx) {
			...
			for ( var i = 0; i < it.length; i++) {
				...
				arr[i] = new ItemMinibar(_id, _name, _code, _amount, _price, _status, _date, _usermodify);
			}
			table.load(arr);
...
function ItemMinibar(id, name, code, amount, price, status, date, usermodify) {
	this.Id = id;
	this.Name = name;
	this.Code = code;
	this.Amount = amount;
...
function addItemMinibar() {
	...
	this.show = function() {
		layer.show();
		var url = "SvcHouseKeeping?CMD=11";
		url += "&r=" + Math.random();
		var f = new AjaxGetText(url);
		f.complet = function(tx) {
			layer.addHtml(tx);
	...
	this.accept = function() {
		...
		f.complet = function(tx) {
			if(tx == "t"){
				...
				var albox = new alertBox();
				albox.show("Add item minibar error???");
			}
		};
	};
	...
	this.accept = function(id) {
		...
		f.complet = function(tx) {
			if(tx == "t"){
				...
				var albox = new alertBox();
				albox.show("Edit item minibar error???");
			}
		};
	};
	this.update = function() {
