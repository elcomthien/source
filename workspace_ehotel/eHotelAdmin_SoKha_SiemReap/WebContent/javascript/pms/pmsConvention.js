...
function subject() {
	...
	this.addSubject = function(id) {
		// abc
		var textbox = new formSubjectPMS(langpms.addSubject, "", "", "");
		textbox.show();
	...
	this.editsubject = function(id) {
		...
		var im1 = obj.Image;
		var im2 = obj.ImageBG;
		var textbox = new formSubjectPMS(langpms.editSubject, obj.Name, im1,
				im2);
		textbox.show();
		textbox.accept = function(value, image1, image2) {
			if (value.length == 0) {
	...
	this.reload = function(id) {
		...
		f.complet = function(tx) {
			...
			menu.reload(arr);
			if (my.List.length == 0) {
				var html = "<a href='javascript:hotel.sub.addSubject(-1);'class='div_addeImage'>"
						+ langpms.addSubject + "</a>";
				// $("div_addsubject").html(html);
				document.getElementById("div_addsubject").innerHTML = html;
				return;
			}
			my.selectID(id);
		};
