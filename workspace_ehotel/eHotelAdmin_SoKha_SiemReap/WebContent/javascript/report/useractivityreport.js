var useractirpt=new function UserActivityReport()
{
	var my=this;
	var table=new Table();
//	var Index=0;
	...
	table.dataBind=function()
 	{
		...
		html += "No";
		html += "</th>";
		html += "<th align='left'  style='text-align: left;'  width=\"" + 55 + "%\"  >";
		html += "Description";
		html += "</th>";
	...
	this.get = function() {
		...
		var url = "UserActivitiesReport?CMD=1";
		url += "&from=" + from;
		url += "&to=" + to;
		url += "&r=" + Math.random();
...
//			var xml = tx.getElementsByTagName("xml");
			...
			for ( var i = 0; i < it.length; i++) {
				...
				arr[i] = new itemData(_des, _date, _user);
			}
			my.List = arr;
			table.load(my.List);
		};
	};
	function itemData(des,date,user)
	{
		this.Des = des;
		this.Date = date;
		this.User = user;
