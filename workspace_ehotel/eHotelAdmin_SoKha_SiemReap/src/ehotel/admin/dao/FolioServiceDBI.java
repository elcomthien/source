...
public class FolioServiceDBI {
	...
	public static final String ADD_MESSAGE_ALL_FOLIO = "BEGIN EPMS.addMessageAllFolio(?,?,?); END;";
	...
	public static final String GET_ALL_FOLIO = "BEGIN EPMS.getAllRoom(?); END;";
	...
	public boolean insertMessageAllFolio(String subject, String content, String sender) {
		boolean flag = true;
		Vector<SubProParam> param = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new String(subject), SubProParam.IN);
		param.add(in);// 1
		...
		param.add(in);// 3
		try {
			broker.executeSubPro(ADD_MESSAGE_ALL_FOLIO, param);
		} catch (Exception e) {
			e.printStackTrace();
			flag = false;
		}
	...
	public boolean insertMessageListFolio(String listroom, String subject, String content, String sender) {
		boolean flag = true;
		Vector<SubProParam> param = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new String(listroom), SubProParam.IN);
		param.add(in);// 1
		...
		param.add(in);// 3
		try {
			broker.executeSubPro(ADD_MESSAGE_LIST_FOLIO, param);
		} catch (Exception e) {
			e.printStackTrace();
			flag = false;
		}
	...
	public List<GuestModel> getGuestForFolio(int id) {
		...
		String sql = "select FIRSTNAME, LASTNAME, CLIENT_ID, TO_CHAR(ARRIVAL_TIME, 'dd/mm/yy') as ARRIVAL_TIME , TO_CHAR(DEPARTMENT_TIME, 'dd/mm/yy') as DEPARTMENT_TIME, CLIENT_NAME, CLIENT_STATUS, ROOM_SHARER, PINCODE, (select PASS_CELLPHONE from pms_folio where folio_code = "
				+ id + ") AS UNIQUEPIN from pms_guestprofile where folionum = " + id + " and client_status = 1 order by CLIENT_ID asc";
		Vector vector = new Vector();
		try {
			vector = broker.executeSelect(sql, null);
			if (vector.size() > 2) {
	...
	public static String getURLPromotion(int id, int lang) {
		String link = "";
		String sql = "select url_web from pms_promotion where promotion_id = " + id + " and lang_id = " + lang;
		Vector vector = new Vector();
		vector = broker.executeSelect(sql, null);
		if (vector.size() > 2) {
			ResultDB result = new ResultDB(vector);
	...
	public List<FolioModel> getListFolio(int from, int to) {
		...
		for (int i = 0; i < outParam.size(); i = i + 9) {
			...
			folio.setRoomstatus(outParam.get(i + 8));
			list.add(folio);
		}
		return list;
	}
	...
	public List<String> getAllRoom(){
		...
		params.add(subOut);
		try {
			params = broker.executeSubPro(GET_ALL_FOLIO, params);
			if ((params != null) & (params.size() > 0)) {
				subOut = (SubProParam) params.get(0);
				outParam = subOut.getVector();
			}
	...
	public static void main(String[] args) {
		...
		System.out.println(f.getAllRoom());
	}
}
