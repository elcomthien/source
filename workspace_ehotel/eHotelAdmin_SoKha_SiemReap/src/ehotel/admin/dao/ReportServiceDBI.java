...
import ehotel.admin.Report.InternetReportModel;
...
public class ReportServiceDBI {
	...
	public List<VodReportModel> getMonthlyStaticReport(String year) {
		...
		String sql = "select to_char(starttime,'MM/yyyy') monthly, count(*) AMOUNT , sum(samount) sotien "
				+ "from vod_charge where servicetypename = 'VOD' and to_char(starttime,'yyyy') = " + year
				+ "  group by to_char(starttime,'MM/yyyy')";
		Vector vector = new Vector();
		try {
			vector = broker.executeSelect(sql, null);
			if (vector.size() > 2) {
	...
	public List<RoomReportModel> getRoomReport(String from, String to) {
		...
		String sql = "select DISTINCT s.FOLIO_CODE, v.CONTENTNAME, to_char(r.VIEWDATE, 'DD-MON-YY HH24:MI:SS') as VIEWDATE, v.IUNIT, v.CURRENCY from VOD_REPORT r, VOD_CONTENTS v, PMS_FOLIOSTB s where r.CONTENTID = v.CONTENTID and r.SERINUMBER = s.SMART_CARD and r.VIEWDATE >= TO_TIMESTAMP ('"
				+ from
				+ " 00:00:00', 'DD/MM/YYYY HH24:MI:SS') and r.VIEWDATE <= TO_TIMESTAMP ('"
				+ to
	...
	@SuppressWarnings("rawtypes")
	public List<RoomServiceOrderModel> getServiceOrderReport(String from, String to) {
	...
	@SuppressWarnings("rawtypes")
	public List<String> getRoomInternet(String from, String to) {
		List<String> list = new ArrayList<String>();
		String sql = "select s.folio_code from vod_charge_internet i, pms_foliostb s where i.sn = s.smart_card and i.time_post >= TO_TIMESTAMP('"
				+ from
				+ " 00:00:00','DD/MM/YYYY HH24:MI:SS') and i.time_post <= TO_TIMESTAMP('"
				+ to
				+ " 23:59:59', 'DD/MM/YYYY HH24:MI:SS')";
		Vector vector = new Vector();
		vector = broker.executeSelect(sql, null);
	...
	@SuppressWarnings("rawtypes")
	public List<InternetReportModel> getInternetReport(String from, String to) {
		List<InternetReportModel> list = new ArrayList<InternetReportModel>();
		String sql = "select i.id,s.folio_code,i.sn,i.price,to_char(i.time_post, 'DD-Mon-YYYY HH:MI AM') as timepost from vod_charge_internet i, pms_foliostb s where i.sn = s.smart_card and i.time_post >= TO_TIMESTAMP('"
				+ from
				+ " 00:00:00','DD/MM/YYYY HH24:MI:SS') and i.time_post <= TO_TIMESTAMP('"
				+ to
				+ " 23:59:59', 'DD/MM/YYYY HH24:MI:SS') order by TO_NUMBER(s.FOLIO_CODE)";
		...
		if (vector.size() > 2) {
			...
			while (result.hasNext()) {
				result.next();
				InternetReportModel in = new InternetReportModel();
				in.setId(result.getParam("ID"));
				in.setRoom(result.getParam("FOLIO_CODE"));
				in.setSerinumber(result.getParam("SN"));
				in.setPrice(result.getParam("PRICE"));
				in.setTime(result.getParam("TIMEPOST"));
				list.add(in);
			}
		}
		return list;
	...
	public static void main(String[] args) {
		...
		System.out.println(r.getRoomInternet("01/05/2017", "06/06/2017"));
		System.out.println(r.getInternetReport("01/05/2017", "06/06/2017"));
	}
}
