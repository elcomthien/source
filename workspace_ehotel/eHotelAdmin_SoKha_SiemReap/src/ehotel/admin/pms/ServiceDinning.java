...
import ehotel.admin.dao.DiningDao;
...
public class ServiceDinning extends ServiceParent {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case 6:// get item restaurance
		{
			...
			if (request.getParameter("pageIndex") != null) {
				pageIndex = Integer.parseInt(request.getParameter("pageIndex").toString().trim());
			}
			if (request.getParameter("page") != null) {
				page = Integer.parseInt(request.getParameter("page").toString().trim());
			}
			String st = getItemPromotion(subId, pageIndex, page);
			out.print(st);
			break;
		}
		case 7:// detail restaurant
		{
			...
			this.showJSPpage(request, response, "/pmsMng/Dinning/detaipromotion.jsp");
			break;
		}
		case 9:// break;
		{
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case 8:// insert item promotion
		{
			System.out.println("Insert item promotion");
			String name = "";
			String image = "";
			String price = "";
			String sale = "";
			...
			if (request.getParameter("subid") != null) {
				subid = request.getParameter("subid").toString().trim();
			}
			if (request.getParameter("price") != null) {
				price = request.getParameter("price").toString().trim();
			}
			if (request.getParameter("sale") != null) {
				sale = request.getParameter("sale").toString().trim();
			}
			if (request.getParameter("iunit") != null) {
				iunit = request.getParameter("iunit").toString().trim();
			}
			...
			def = def.replaceAll("<span style=\"text-decoration: underline;\">", "<u>");
			def = def.replaceAll("</span>", "</u>");
			// def = def.replaceAll("<p>", "").replaceAll("</p>", "");

			name = UnicodeConverter.encodeUnicode(name);
			def = UnicodeConverter.encodeUnicode(def);

			DiningPromotionModel itm = new DiningPromotionModel();
			itm.setSubject(subid);
			itm.setName(name);
			itm.setDef(def);
			itm.setNewPrice(sale);
			itm.setOldPrice(price);
			itm.setIunit(iunit);
			itm.setImage(config._dining + "/" + image);
			int t = diningDao.addItemDingningPromotion(itm);
		...
		case 9:// edit item promotion
		{
			System.out.println("Edit item promotion");
			String name = "";
			String image = "";
			String price = "";
			String sale = "";
			...
			itm.setIunit(iunit);
			itm.setImage(config._dining + "/" + image);
			itm.setLangId(String.valueOf(LangID));
			int t = diningDao.editItemDingningPromotion(itm);
		...
		case 10:// delete item promotion
		{
			System.out.println("Delete item promotion");
			String id = "";
			if (request.getParameter("id") != null) {
				id = request.getParameter("id").toString().trim();
			}
			int t = diningDao.deleteItemDiningPromotion(id);
			if (t > 0)
				out.print(true);
			else
				out.print(false);
		...
		case 11:// change invisible item promotion
		{
			System.out.println("Change invisible item promotion");
			String id = "";
			if (request.getParameter("id") != null) {
				id = request.getParameter("id").toString().trim();
			}
			int t = diningDao.changeInviItemDiningPromotion(id);
			if (t > 0)
				out.print(true);
			else
				out.print(false);
	...
	private String getItemPromotion(int id, int index, int page) {
		...
		list = diningDao.getListDiningPromotion(id, LangID, fr, to);
		String mData = "";
		mData += "<?xml version=\"1.0\" encoding=\"ISO-8859-1\" standalone=\"yes\" ?>\n";
		mData += "<xml>";
		for (int i = 0; i < list.size(); i++) {
			DiningPromotionModel item = list.get(i);
			mData += "<Item>";
			mData += "<name>";
			mData += "<![CDATA[" + UnicodeConverter.decodeUnicode(item.getName()) + "]]>";
			mData += "</name>";
			mData += "<id>";
			mData += item.getId();
			mData += "</id>";
			...
			if (item.getDef() == null) {
				...
				mData += "<![CDATA[" + UnicodeConverter.decodeUnicode(item.getDef()) + "]]>";
			mData += "</def>";
			mData += "<newprice>";
			mData += item.getNewPrice();
			mData += "</newprice>";
			mData += "<oldprice>";
			mData += item.getOldPrice();
			mData += "</oldprice>";
			mData += "<iunit>";
			mData += item.getIunit();
			mData += "</iunit>";
			...
			mData += "</invisible>";
			mData += "<image><![CDATA[";
			mData += config._linksaveimage + "/" + item.getImage();
			mData += "]]></image>";
			mData += "</Item>";
		}
		mData += "</xml>";
	...
	public static void main(String[] args) {
		DiningPMS d = new DiningPMS();
		System.out.println(d.getRestaurantItems(325, 2, -1, -1));
	}
}