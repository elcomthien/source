	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case 10:// remove
		{
			...
			if (iuser.isAdmin(ipAdress)
				...
				String chuoi = "(" + vodId + ")";
				if(subId == 547){
					String[] arr=vodId.split(",");
					HttpSession session = request.getSession(true);
					String user = "";
					user = session.getAttribute("user").toString();
					for(int i = 0;i<arr.length;i++){
						String name = VodContentServiceDBI.getContentNameContentById(arr[i]);
						ReportServiceDBI.addUserActivities("Delete charged movie " + name, user);
					}
				}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case 1: {
			...
			if (request.getParameter("name") != null) {
				...
				// image,-1);
				int id = srvMod.addSubject(subjectName, image, 4);
				if (id > 0) {
