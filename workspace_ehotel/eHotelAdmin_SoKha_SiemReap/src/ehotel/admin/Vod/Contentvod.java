	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			if (list.size() > 0) {
				...
				try {
					...
					for (int i = 0; i < list.size(); i++) {
						Float temp = (float) (ftpgateway.transfered(list.get(i)
								.getUid()) * 100)
									...
									.parseInt(list
											...
											.substring(
													...
													list.get(i).getFilename()
															.length() - 4));
							vodContentServiceDBI.updateStatusInvisibleVod(
									vodId, list.get(i).getFilename());
						}
					}
				} catch (Exception e) {
					e.printStackTrace();
		...
		case 4:// delete Movie
		{
			...
			if (iuser.isAdmin(ipAdress)
				...
				if (request.getParameter(Def.VodId) != null) {
					VodId = Integer.parseInt(request.getParameter(Def.VodId)
							.toString());
				}
		...
		case 10:// add subtitle
		{
			...
			while (request.getParameter("lang" + i) != null) {
				int l = Integer.parseInt(request.getParameter("lang" + i)
						.toString().trim());
				lang.add(l);
				i++;
			}
			...
			while (request.getParameter("name" + i) != null) {
				...
				String ipserver = vodContentServiceDBI.getIpserver(VodId);
				String type = "";
				if (ipserver.equals("172.17.20.12"))
					type = "/12";
				else if (ipserver.equals("172.17.20.13"))
					type = "/13";
				transferVod(n, namesave, "", hostsrc, portsrc, usersrc,
						passsrc, type);
			...
			for (i = 0; i < lang.size(); i++) {
				System.out.println("lang:" + lang.get(i) + " ID:" + ID.get(i)
						+ " Name:" + name.get(i));
			...
			for (i = 0; i < subtitle.size(); i++) {
				String filetype = subtitle
						.get(i)
						.getUrl()
						.substring(subtitle.get(i).getUrl().lastIndexOf("."),
								subtitle.get(i).getUrl().length());
				String namesave = VodId + "_" + getLang(lang.get(i)) + filetype;
				vodContentServiceDBI.addSubtitleDbi(VodId, namesave, subtitle
						.get(i).getLangId());
		...
		case 11:// delete subtitle
		{
			...
			while (request.getParameter("id" + i) != null) {
				int id = Integer.parseInt(request.getParameter("id" + i)
						.toString().trim());
				// System.out.println("id content vod for delete sub = = = = "
				// + id);
				String ipserver = contentServiceDBI.getIpserver(id);
				String filename = contentServiceDBI.getSubVodById(id);

				// System.out.println("delete sub:  = = = = = " + filename);
				deleteFileSubInFtpServer(filename, ipserver);
		...
		case 16: {
			...
			if (list.size() > 0) {
				...
				list = contentServiceDBI.getVodTransferPercent();
				for (int i = 0; i < list.size(); i++) {
					rs += list.get(i).getFilename() + ",,,"
							+ list.get(i).getUid() + ",,,"
							+ list.get(i).getNameview() + ",,,";
				}
				if (rs != "")
		...
		case 19: {
			...
			int flag = vodContentServiceDBI.checkFilmName(name);
			if (flag > 0)
				out.print("success");
			else
				out.print("unsuccess");
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case 1: {
			...
			if (request.getParameter("name") != null) {
				...
				VOD vodDBI = DBIGateway.getAMDVod();
				AMDVod ctnMod = vodDBI.getAMDCntVod();
				int id = ctnMod.addSubject(subjectName, config._movie + "/"
						+ image, parent);
				if (id > 0) {
		...
		case 2:// update subject
		{
			...
			if (request.getParameter("name") != null) {
				if (request.getParameter(Def.SubId) != null) {
					...
					VOD vodDBI = DBIGateway.getAMDVod();
					AMDVod ctnMod = vodDBI.getAMDCntVod();
					int t = ctnMod.editSubject(subjId, subjectName,
							config._movie + "/" + image, LangID);
					out.print(t);
					if (t > 0) {
						ManagerFile file = new ManagerFile();
						String path1 = config._temp + "/" + image;
		...
		case 5: {
			...
			String pathFile = getServletContext().getRealPath("")
					...
					+ "data.txt";
			String text = configService.ReadFile(pathFile);
			System.out.println("Text of file data.txt: " + text);
			if (!text.equals("")) {
				String[] arr = text.split(",");
				hostsrc = arr[0];
				portsrc = Integer.parseInt(arr[1]);
			...
			System.out.println("host: " + hostsrc + " - port: " + portsrc + " - user: " +usersrc + " - pass: " + passsrc);
			...
			vod.setPlot(def);
			vod.setPoster(image);
			vod.setFilePath(filetype);
			vod.setCurrency(price);
			vod.setIUnit(money);
			System.out.println("add vod");
			addVod(subId, vod);
			System.out.println("add vod success");
			String maxid = contentServiceDBI.getMaxIdVodContent();
			// contentServiceDBI.updateUrlForVod(Integer.parseInt(maxid), maxid
			// + "." + filetype);
			String type = "";
//			int mem11 = geHDD11();
//			System.out.println("HDD11: " + mem11);
//			int mem12 = getHDD12();
//			System.out.println("HDD12: " + mem12);
//			int mem13 = getHDD13();
//			System.out.println("HDD13: " + mem13);
...
//			}
			type = "/13";
			System.out.println("folder save: " + type);

			boolean rs = transferVod(file, maxid + "." + filetype, name,
					hostsrc, portsrc, usersrc, passsrc, type);
			...
			if (rs) {
				...
				vodContentServiceDBI.deleteVodContentById(Integer
						.parseInt(maxid));
			}
			out.print(rs);
			break;
	...
	public int addVod(int subjId, Vod vod) {
		int seq = -1;
		int storageid = 1;
		String ipserver = "172.17.20.11";
//		int mem11 = geHDD11();
//		int mem12 = getHDD12();
//		int mem13 = getHDD13();
//		if (mem11 > 95) {
//			if (mem12 < 95) {
//				ipserver = "172.17.20.12";
//			} else {
//				if (mem13 < 95) {
//					ipserver = "172.17.20.13";
//				} else {
//					return -1;
//				}
//			}
//		}
		ipserver = "172.17.20.13";
		vodContentServiceDBI.addVodDbi(subjId, storageid, vod.getFilePath(),
				vod.getTitle(), vod.getDirector(), vod.getActors(),
				vod.getPlot(), vod.getPoster(), vod.getCurrency(),
				vod.getIUnit(), "0", ipserver, seq);
		return seq;
	}

	public MetadataVideo readMetaDataVideo(String fileName) {
		System.out.println("Reading metadata video[fileName=" + fileName
				+ "url=" + URLConstant.MEDIAPARSE + "]");
		MetadataVideo video = null;
		try {
			MediaFileParserInterface mediaParse = (MediaFileParserInterface) Naming
					.lookup(URLConstant.MEDIAPARSE);
			...
			if (video != null)
				...
				System.out
						.println("mediaParse return value null.Maybe not found file");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return video;
	...
	private void updateVod(HttpServletRequest request,
		...
		if (value != "-1") {
			try {
				...
				for (int i = 0; i < arr.length; i = i + 3) {
					int vodId = Integer.parseInt(arr[i].substring(0,
							arr[i].length() - 4));
					if (ftpgateway.getStatus(UUID.fromString(arr[i + 1])) == FTPServerStruct.STATUS_COMPLETED) {
						System.out.println("Transfer = = = = = = = = = = 100%");
						vodContentServiceDBI.updateStatusInvisibleVod(vodId,
								arr[i]);
						arr = deleteArray(arr, i);
					} else {
						float percentfloat = (float) (ftpgateway
								.transfered(UUID.fromString(arr[i + 1])) * 100)
								/ ftpgateway.fileSize(UUID
										.fromString(arr[i + 1]));
						Float temp = (float) (ftpgateway.transfered(UUID
								.fromString(arr[i + 1])) * 100)
								/ ftpgateway.fileSize(UUID
										.fromString(arr[i + 1]));
						Float compare = Float.NaN;

						if (temp.compareTo(compare) != 0) {
							System.out.println("percent of uuid: " + arr[i + 1]
									+ " = = = = = " + percentfloat);
	...
	public boolean transferVod(String filepathSrc, String filenameServer,
			String nameview, String hostSrc, int portSrc, String userSrc,
			String passSrc, String type) {
		boolean flag = true;
		ConfigLoader loader = new ConfigLoader();
		Config config = loader.getConfig();
		String hostserver = config._HostFtpServer;
		...
		try {
			...
			ftpgateway = (FTPGatewayInterface) Naming.lookup("rmi://"
					+ hostserver + ":2099/elc_ftpgateway");
			System.out.println("RMI = " + "rmi://" + hostserver
					+ ":2099/elc_ftpgateway");
			...
			FTPServerStruct server = new FTPServerStruct(hostSrc, portSrc,
					userSrc, passSrc, filepathSrc);
			System.out.println("file path inserver = = = = = " + filepathserver
					+ "/" + foldername + "/" + filenameServer);
			UUID id = ftpgateway.download(server, hostserver, filepathserver
					+ "/" + foldername + "/" + filenameServer, 5000);
			System.out.println("id uuid = = = " + id);
			String file = filenameServer.substring(filenameServer
					.lastIndexOf(".") - (-1));
			if ("mp4".equalsIgnoreCase(file) || "mkv".equalsIgnoreCase(file)
					|| "mov".equalsIgnoreCase(file)
					|| "flv".equalsIgnoreCase(file)
					|| "avi".equalsIgnoreCase(file))
				if (id != null) {
					nameview = nameview.replaceAll("'", " i");
					contentServiceDBI.insertVodRemote(filenameServer, id,
							nameview);
				} else {
					int vodId = Integer.parseInt(filenameServer.substring(0,
							filenameServer.lastIndexOf(".")));
	...
	public boolean deleteTransferVod(String uuid, String filename) {
		...
		try {
			...
			// boolean temp = ftpgateway.deleteFile(hostserver, filename);
	...
	public boolean deleteFileInFtpServer(int VodId) {
		...
		flag = deleteFileFTPSupport(filename, ipserver);
		if (!flag)
			return false;
		for (int i = 0; i < listsub.size(); i++) {
			flag = deleteFileFTPSupport(listsub.get(i), ipserver);
			if (!flag)
				return false;
		}
		if (flag)
	...
	public boolean deleteFileFTPSupport(String filename, String ipserver) {
		ConfigLoader loader = new ConfigLoader();
		Config config = loader.getConfig();
		...
		String user = config._UserSubtitle;
		String pass = config._passSubtitle;
		System.out.println(ipserver + " - " + port + " - " + user + " - "
				+ pass);
		System.out.println("delete file: " + filename + " - in folder: "
				+ folder);
		boolean flag = true;
		try {
			Ftp4jClient ftp4j = new Ftp4jClient(ipserver, port, user, pass);
			ftp4j.connect();
	...
	public boolean deleteFileSubInFtpServer(String filename, String ipserver) {
		ConfigLoader loader = new ConfigLoader();
		Config config = loader.getConfig();
		// String host = config._HostFtpServer;
		int port = config._PortFtpServer;
	...
	public int geHDD11() {
		...
		String info = replaceSpace(result);
		String[] arr = info.split(" ");
		int usedper = Integer.parseInt(arr[4]);
		return usedper;
	}
	...
	public int getHDD12() {
		...
		try {
			JSch jsch = new JSch();
			Session session = jsch.getSession("root", "172.17.20.12", 7890);
			session.setPassword("siemreap02#@");
	...
	public int getHDD13() {
		...
		try {
			JSch jsch = new JSch();
			Session session = jsch.getSession("root", "172.17.20.13", 7890);
			session.setPassword("siemreap03#@");
			Properties config = new Properties();
			config.put("StrictHostKeyChecking", "no");
			session.setConfig(config);
			session.connect();
	...
	public static void main(String[] args) {
		// Contentvod c = new Contentvod();
		// c.deleteFileSubInFtpServer("Zombieland.REPACK.2009.720p.Blu-ray.DTS.x264-CROSSBOW.srt");
		// c.addVodDbi(1, 1, "asd", "das", "asd", "asd", "da", "asd", "asd",
		// "as", "sda", 0);
		String a = "abv.123";
