...
public class ServiceSystem extends ServiceParent {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case 1: {
			System.out
					.println("Khoi tao: Lay tat ca service trong ServiceSystem");
			int index = 0;
			int page = 6;
			int langId = 1;
			if (request.getParameter("lang") != null) {
				langId = Integer.parseInt(request.getParameter("lang")
						.toString().trim());
			}
		...
		case 3: {
			...
			this.showJSPpage(request, response,
					"/system/detailServiceSystem.jsp");
			break;
		}
		...
		case 5: {
			System.out
					.println("Lay tat ca service trong ServiceSystem cho ORDER");
			int index = 0;
			int page = 6;
			int langId = 1;
			if (request.getParameter("lang") != null) {
		...
		/*
		 * lay ra cac dich vu may ao dang chay, tong cong co 11 12 may ao dang
		 * chay dbi, dbora, eas, ehotelmanservice, ehotelservice, ftpclient,
		 * ftpgateway, ievsserver, initVLC, livetv, lsnrora, mediafileparse
		 * monitorcenter, pmscom, rmc, srvmonitor
		 */
		case 6: {
		...
		case 7: {
			...
			System.out.println("Thay doi trang thai status cua module: " + name
					+ " chuyen sang " + type);
			response.setContentType("text/xml");
		...
		case 8: {
			...
			String url = request.getScheme() + "://" + request.getServerName()
					+ ":" + request.getServerPort() + "/eHotelAdmin";
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case 3: {
			...
			if (request.getParameter("lang") != null) {
				lang = Integer.parseInt(request.getParameter("lang").toString()
						.trim());
			}
			boolean b = main.updateServiceMain(id, name, config._main + "/"
					+ image, LangID);
			if (b) {
	...
	// 4 update order public boolean orderByService(String str_service_id,
	// String str_order)
	...
	private String getServices(int index, int page, int langId) {
		...
		// ap dung cho philao: bo HK 6, Game 10, Wakeup 14 va Internet 9:
		// 13.03.13
	...
	private ArrayList<HashMap<String, String>> getDaemon() {
		...
		if (isUnix()) {
			...
			String[] listCmd = new String[] { "/bin/bash", "-c",
					"ls /etc/init.d/eod* /etc/init.d/httpd /etc/init.d/vsftpd -l" };
			...
			try {
				...
				while ((lsString = bufferedReader.readLine()) != null) {
					...
					String name = lsString
							.substring(lsString.lastIndexOf(" ") + 1);
					name = name.substring(name.lastIndexOf("/") + 1);
					if (checkServiceValid(name) == 1) {
						if (!(name.equalsIgnoreCase("eod_wer"))
								&& !(name.equalsIgnoreCase("eod_syndatabase"))
							...
							Process runtimeProcess1 = Runtime.getRuntime()
									.exec(executeCmd);
							BufferedReader bufferedReader1 = new BufferedReader(
									new InputStreamReader(
											runtimeProcess1.getInputStream()));
							if (bufferedReader1.readLine() != null)
								mp.put("status", "0");
							else
								mp.put("status", "1");
							if (name.equalsIgnoreCase("httpd")) {
								String[] executeCmd1 = new String[] {
										"/bin/bash",
										"-c",
										"lsof -i -n -P | grep "
												+ prop.getProperty(name) };
								Process runtimeProcess11 = Runtime.getRuntime()
										.exec(executeCmd1);
							...
							result.add(mp);
						} else if (name.equalsIgnoreCase("eod_wer")) { // get
																		// status
																		// for
																		// weather
							HashMap<String, String> mp = new HashMap<String, String>();
							...
							String[] executeCmd = new String[] { "/bin/bash",
									"-c", "ps -aux | grep WeatherSynApp.jar" };
							Process runtimeProcess1 = Runtime.getRuntime()
									.exec(executeCmd);
							BufferedReader bufferedReader1 = new BufferedReader(
									new InputStreamReader(
	...
	private String changeStatusModule(String name, String type) {
		...
		if (isUnix()) {
			...
			if (name.equalsIgnoreCase("eod_dbi_02")
					&& type.equalsIgnoreCase("start")) {
				String[] Cmd = new String[] { "/bin/bash", "-c",
						"service eod_ehotelmanservice start" };
			...
			if (name.equalsIgnoreCase("eod_dbi_01")
					&& type.equalsIgnoreCase("start")) {
				String[] Cmd = new String[] { "/bin/bash", "-c",
						"service eod_ehotelservice start" };
	...
	public boolean writeToFileJnlp(String port) {
		...
		try {
			...
			out.write("<jnlp spec=\"1.0+\" codebase=\"http://"
					+ prop.getProperty("ehotel.module.host")
			...
			out.write("<argument>" + prop.getProperty("ehotel.module.host")
					+ "</argument>\n");
	...
	private String getServicesNameXml() {
		System.out
				.println("Get service name to xml!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
	...
	public String getMemoryUsage() {
		...
		System.out.println("getMemoryUsage: " + result);
		return result;
	}

	// 2286767 1853387 317213 86% /data
	public String getInfoMemory() {
		String info2 = getMemoryUsage();
		String info8 = getHDD8();
		String info12 = getHDD12();
		
		// String info =
		// "                        2286767 1853387    317213  86% /data";
		info2 = replaceSpace(info2);
		info8 = replaceSpace(info8);
		info12 = replaceSpace(info12);
		String[] arr2 = info2.split(" ");
		String[] arr8 = info8.split(" ");
		String[] arr12 = info12.split(" ");
		
		
		
		long used = Long.parseLong(arr2[2]) + Long.parseLong(arr8[2]) + Long.parseLong(arr12[2]);
		long free = Long.parseLong(arr2[3]) + Long.parseLong(arr8[3]) + Long.parseLong(arr12[3]);
		long total = used + free;
		int usedper = (int)((used * 100)/total);
		...
		System.out.println("xml getInfoMemory: " + data);
		return data;
	}
	...
	public String getRamUsage() {
		...
		System.out.println("getRamUsage: " + result);
		return result;
	}
...
//		System.out.println("xml getInfoRam: " + data);
		return data;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(s.getServices(1, 6, 2));
	}
}