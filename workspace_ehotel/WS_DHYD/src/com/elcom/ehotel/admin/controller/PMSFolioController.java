...
public class PMSFolioController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (action.equals("getlistpopup")) {
			String type = request.getParameter("type");
			LogUtil.logControl(PMSFolioController.class.toString(), "getlistpopup", "type,,," + type);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (action.equals("addpopup")) {
			String name = request.getParameter("name");
			String def = request.getParameter("url");
			String timeout = request.getParameter("timeout");
			String type = request.getParameter("type");

			LogUtil.logControl(PMSFolioController.class.toString(), "addpopup", "name,,," + name + ",,,def,,," + def + ",,,timeout,,,"
					+ timeout + ",,,type,,," + type);

			int rs = pmsFolioService.addPopup(name, def, timeout, type);
			out.println(rs);
		}

		if (action.equals("editpopup")) {
			String id = request.getParameter("id");
			String name = request.getParameter("name");
			String def = request.getParameter("url");
			String timeout = request.getParameter("timeout");

			LogUtil.logControl(PMSFolioController.class.toString(), "editpopup", "id,,," + id + ",,,name,,," + name + ",,,def,,," + def
					+ ",,,timeout,,," + timeout);

			int rs = pmsFolioService.editPopup(id, name, def, timeout);
			out.println(rs);
		}

		if (action.equals("deletepopup")) {
			String id = request.getParameter("id");

			LogUtil.logControl(PMSFolioController.class.toString(), "deletepopup", "id,,," + id);

			int rs = pmsFolioService.deletePopup(id);
			out.println(rs);
		}

		if (action.equals("postpopup")) {
			String id = request.getParameter("id");
			String listroom = request.getParameter("listroom");

			LogUtil.logControl(PMSFolioController.class.toString(), "postpopup", "id,,," + id + ",,,listroom,,," + listroom);

			int rs = pmsFolioService.postPopup(id, listroom);
			out.println(rs);
		}
	}
