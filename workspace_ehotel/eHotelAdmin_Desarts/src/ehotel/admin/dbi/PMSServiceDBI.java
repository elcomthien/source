...
public class PMSServiceDBI {
	...
	public static final String UPDATE_INDEX_MENU_DINING = "BEGIN EPMS.updateIndexDiningMenu(?,?); END;";
	...
	public List<PMSMainMenu> getMenuDining(int id, int langid) {
		List<PMSMainMenu> list = new ArrayList<PMSMainMenu>();
		String sql = "select m.menuno,m.menuname,url.url_image,url.url_background,url.url_picicon,m.parent_id,m.menu_type, m.menuactive,m.orderby "
				+ "from PMS_MENU m left join pms_imageurl url on m.image_id=url.image_id "
				+ "where m.mmmenuno=" + id + " and lang_id=" + langid + " order by m.orderby";
		Vector vector = new Vector();
		vector = broker.executeSelect(sql, null);
		if (vector.size() > 2) {
			ResultDB result = new ResultDB(vector);
			while (result.hasNext()) {
				...
				menu.setMenuindex(result.getParam("orderby"));
	...
	public boolean updateIndexMenuDining(int id, String index) {
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(id), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(index), SubProParam.IN);
		params.add(in);
		try {
			broker.executeSubPro(UPDATE_INDEX_MENU_DINING, params);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
