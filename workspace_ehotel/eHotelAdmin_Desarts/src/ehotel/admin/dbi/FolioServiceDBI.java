...
public class FolioServiceDBI {
	...
	public static final String UPDATE_MESSAGE = "BEGIN EPMS.UPDATEMESSAGEFOLIO(?,?,?,?); END;";

	@SuppressWarnings("unchecked")
	public boolean CheckIn(int folionum, String firstname, String lastname, String gioitinh, String checkindate, String checkoutdate,
			int roomsharer) {
		// String sql =
		...
		SubProParam in = new SubProParam(new BigDecimal(folionum), SubProParam.IN);
		params.add(in);
		...
		try {
			...
			if (vector.size() > 2) {
				...
				while (result.hasNext()) {
					...
					record.setPrivatechannelname(result.getParam("PRIVATE_CHANNELNAME"));
					if (!result.getParam("SIZEINKB").equalsIgnoreCase(""))
						record.setSizeinkb(Integer.parseInt(result.getParam("SIZEINKB")));
	...
	public Vector<Record> getRecordForFolio(String folio) {
		...
		if (listsernumber.size() == 0)
			...
			String sql = "select * from record_channel where " + condition + " order by id asc";
			Vector vector = new Vector();
			try {
				vector = broker.executeSelect(sql, null);
				if (vector.size() > 2) {
	...
	public List<String> getSmartCardForFolio(String folio) {
		...
		String sql = "select smart_card from pms_foliostb where folio_code = '" + folio + "' order by smart_card asc";
		Vector vector = new Vector();
		try {
			vector = broker.executeSelect(sql, null);
			if (vector.size() > 2) {
	...
	public List<GuestModel> getGuestForFolio(int id) {
		...
		String sql = "select * from pms_guestprofile where folionum = " + id + " and client_status = 1 order by CLIENT_ID asc";
		Vector vector = new Vector();
		try {
			vector = broker.executeSelect(sql, null);
			if (vector.size() > 2) {
				...
				while (result.hasNext()) {
					...
					guest.setClientid(Integer.parseInt(result.getParam("CLIENT_ID")));
					...
					guest.setClientstatus(Integer.parseInt(result.getParam("CLIENT_STATUS")));
					guest.setRoomshare(Integer.parseInt(result.getParam("ROOM_SHARER")));
					guest.setPincode(Integer.parseInt(result.getParam("PINCODE")));
					list.add(guest);
				}
			}
		} catch (Exception e) {
	...
	public int checkExistsMainGuest(int id) {
		int rs = 0;
		String sql = "select * from pms_guestprofile where client_status = 1 and folionum =" + id;
		Vector vector = new Vector();
		try {
			vector = broker.executeSelect(sql, null);
			if (vector.size() > 2)
	...
	public List<eSTB> getListSearchSTB(String key) {
		...
		String sql = "select smartcardid,serinumber,description,folio_code,createdate,status "
				...
				+ "where serinumber like '%" + key + "%' or folio_code like '%" + key + "%' or description like '%" + key + "%'";
		Vector vector = new Vector();
		try {
			vector = broker.executeSelect(sql, null);
			if (vector.size() > 2) {
	...
	public String getPassCellPhoneForFolio(int idfolio) {
		String rs = "";
		String sql = "select PASS_CELLPHONE from PMS_FOLIO where FOLIO_ID = " + idfolio;
		Vector vector = new Vector();
		try {
			vector = broker.executeSelect(sql, null);
			if (vector.size() > 2) {
	...
	public boolean setMainGuest(int clientid) {
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(clientid), SubProParam.IN);
		params.add(in);
		try {
	...
	@SuppressWarnings("rawtypes")
	public HashMap<String, String> getPerformanceBox(String key) {
		HashMap<String, String> hmap = new HashMap<String, String>();
		String sql = "select serinumber,ramtotal,ram,cputotal,cpu,hddtotal,hdd,upload,download,to_char(createdate,'dd/MM/yyyy hh:mi:ss') as createdate,new from pms_smartcard where serinumber = "
				+ key;
		Vector vector = new Vector();
		try {
			vector = broker.executeSelect(sql, null);
			if (vector.size() > 2) {
				ResultDB result = new ResultDB(vector);
				result.moveToFirst();
				hmap.put("serinumber", result.getParam("serinumber"));
				hmap.put("ramtotal", result.getParam("ramtotal"));
				hmap.put("ram", result.getParam("ram"));
				hmap.put("cputotal", result.getParam("cputotal"));
				hmap.put("cpu", result.getParam("cpu"));
				hmap.put("hddtotal", result.getParam("hddtotal"));
				hmap.put("hdd", result.getParam("hdd"));
				hmap.put("upload", result.getParam("upload"));
				hmap.put("download", result.getParam("download"));
				hmap.put("createdate", result.getParam("createdate"));
				hmap.put("new", result.getParam("new"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public boolean updateMessageFolio(String id, String sender, String subject, String content) {
		boolean flag = true;
		...
		param.add(in);// 3
		try {
			broker.executeSubPro(UPDATE_MESSAGE, param);
		} catch (Exception e) {
			e.printStackTrace();
			flag = false;
		}
	...
	@SuppressWarnings("rawtypes")
	public List<HashMap<String, String>> getListUpdateAPK(String key) {
		List<HashMap<String, String>> list = new ArrayList<HashMap<String, String>>();
		String sql = "select SUBSTR(name_packet,INSTR(name_packet, '.',-1) + 1) as TEN, ngay || ' ' || gio as NGAY from apk_update_history where SN = "
				+ key + " order by name_packet, id desc";
		Vector vector = new Vector();
		try {
			vector = broker.executeSelect(sql, null);
			if (vector.size() > 2) {
				...
				while (result.hasNext()) {
					result.next();
					if (!name.equals(result.getParam("TEN"))) {
						HashMap<String, String> map = new HashMap<String, String>();
						map.put("name", result.getParam("TEN"));
						map.put("date", result.getParam("NGAY"));
						list.add(map);
					}
					name = result.getParam("TEN");
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void main(String[] args) {
		...
		System.out.println(f.getListUpdateAPK("2014"));
	}
