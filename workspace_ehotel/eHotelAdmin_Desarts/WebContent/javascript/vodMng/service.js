...
var Vod = new function VodContent() {
	...
	this.changeTab = function(id) {
		...
		if (id == 0) {
			...
			$("div_search_vod_service").style.display = "block";
		} else {
			my.loadAdd("one");
			div[1].className = "tab_menu_item_sel";
			div[0].className = "tab_menu_item";
			$("div_search_vod_service").style.display = "none";
		}
		tag = id;
...
function subjectVod() {
	...
	ctMenu.fucchangestatus = function() {
		var id = this.obj;
		var invi = my.setInvisible(id);
		var url = "VodSrvMain?CMD=12";
		url += "&subid=" + id;
		url += "&invisible=" + invi;
		url += "&t=" + Math.random();
	...
	menu.renderEvent = function() {
		...
		for ( var i = 0; i < li.length; i++) {
			...
			item.oncontextmenu = function(e) {
				var invi = my.setInvisible(this.id);
				my.selectID(this.id);
				ctMenu.load(this.id, e, invi);
				return false;
			};
		}
	};
	...
	this.get = function() {
		...
		f.complet = function(tx) {
			...
			for ( var i = 0; i < it.length; i++) {
				...
				arr[i] = new ItemMenuVodSrv(_id, _parent, _name, _image, _invisible);
			}
			my.List = arr;
			menu.load(arr);
			my.shift();
...
function ItemMenuVodSrv(id,parent,name,image,invi)
{	
	this.Id=id;this.Name=name;this.Image=image;this.Parent=parent;this.Invisible=invi;
}
