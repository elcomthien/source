...
var local=new function LocalAttaction()
{
	var my=this;
...
function subject()
{
	...
	ctMenu.fucchangestatus = function() {
		var id = this.obj;
		var invi = my.getInvisible(id);
		var url = "ServiceActi?CMD=10";
		url += "&subid=" + id;
		url += "&invisible=" + invi;
		url += "&t=" + Math.random();
		...
		f.complet = function(tx) {
			if(tx == "t")
				...
				albox.show("Update status invisible vod subject service error!");
			}
		};
	};
	var my=this;
	...
	this.selectID=function(id)
	{
		...
		for(var i=0;i<a.length;i++)
		{
			if(a[i].id==id)
			{
				...
				if(my.getInvisible(a[i].id) == "1"){
					a[i].style.color = "#FF7F50";
				}
			}else
			{
				a[i].style.color="#878888";
				if(my.getInvisible(a[i].id) == "1"){
					a[i].style.color = "#DC143C";
				}
			}
		}		
	...
	this.getInvisible = function(id) {
		var invi = null;
		for(var i = 0; i < my.List.length; i++){
			if(my.List[i].Id == id){
				invi = my.List[i].Invisible;
				break;
			}
		}
		return invi;
	...
	this.reload=function(id)
	{
		...
		{		
			...
			{			
				...
				item.Invisible = _invisible;
				//loai bo menu con dining va shopping cho philao 13.03.13
				/*if (_id.trim() != "9" && _id.trim() != "11") {
					arr[k]=item;
					k++;
				}*/
				arr[i]=item;
...
function List()
{
	...
	{
		var detail=new detailItemAttaction();
	...
	{	
		var changsub=new changeSUBLocal();
		changsub.load(this.obj.Id);
		changsub.update=function()
		{
			my.reload()
		}
	}
	...
	{		
		...
		if(this.List.length==0)
		{
			...
			//sua tai day, goi sai doi tuong "local" moi la su dung attraction, acti la dung cho activities 19.2
			html+="<a href='javascript:local.list.addeImage()' class='div_addeImage' title='Click here add Image'>"+langpms.addItem+"</a>"
			html+="</div>";
		}
		if(my.count>6)
		{
...
{	
	...
	{			
		...
		{
			var albox=new alertBox();
			albox.show(langpms.pls_address);
			return;
		};		
		var url="Attraction?CMD=3";
			url+="&t="+Math.random();
...
function changeSUBLocal()
{
	...
	{			
		...
		var url="Attraction?CMD=5"
			...
			{			
				layer.addHtml(tx);			
				setCenterDIV("formSubject");
				$("pmsActi_button_ok").onclick=function()
				{		
					my.accept();};
				$("pmsActi_button_Cancel").onclick=my.cancel;				
	...
	{			
		...
		{
			var al=new alertBox();
			al.show(langpms.pls_selectsubject);
			return;
		}
		var newid=select.options[select.selectedIndex].value;
		var url="Attraction?CMD=6";
			url+="&id="+my.ItemId;
			url+="&SubId="+newid;
			url+="&r="+Math.random();
			var f=new AjaxGetText(url);			
...
function contextMenu()
{
	...
	this.load=function(obj,e, invi)
	{
		...
		my.draw(posx,posy-10, invi);
	}
	this.draw=function(x,y, invi)
	{		
		my.state=true;
		var html="";
		html+="<ul>";
	...
	this.renderClick=function()
	{
		...
		var itemchangestatus = li[3].getElementsByTagName("a")[0];
		itemchangestatus.onclick = function() {
			my.fucchangestatus();
		};
	}
...
function addLoadEventAttaction()
{ 
 document.body.onclick=local.onclick;  
 $("div_addeSubject").href="javascript:local.sub.addSubject(-1);";
}
