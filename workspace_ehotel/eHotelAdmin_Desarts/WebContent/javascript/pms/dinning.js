...
function subject() {
	...
	ctMenu.fucchangestatus = function() {
		...
		f.complet = function(tx) {
			if (tx == "t")
				...
				albox
						.show("Update status invisible vod subject service error!");
			}
		};
	};
	var my = this;
	...
	this.addSubject = function(id) {
		var textbox = new pmsBoxSubDining(langpms.addSubject, "", "", "", 0);
		textbox.show();
		textbox.accept = function(value, image1, image2, order) {
			if (value.length == 0) {
				var albox = new alertBox();
	...
	this.editsubject = function(obj) {
		...
		var textbox = new pmsBoxSubDining(langpms.editSubject, name, image, bg,
				index);
		textbox.show();
		textbox.accept = function(value, image1, image2, order) {
			if (value.length == 0) {
	...
	this.loadDefault = function(image) {
		...
		$("id_table").innerHTML = "<div class='div_dinningimage'> <img src='"
				+ dininglink + "' width='200px' height='150px' > </div>";
	};
}
