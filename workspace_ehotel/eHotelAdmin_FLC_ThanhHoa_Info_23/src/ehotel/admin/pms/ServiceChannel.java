...
public class ServiceChannel extends ServiceParent {
	...
	private LiveTvServiceDBI liveTvServiceDBI = new LiveTvServiceDBI();
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case -1:
			...
			request.setAttribute("fileJSP", "../serviceMng/ChannelAdvertise.jsp");
		...
		case 15: {
			...
			response.setContentType("text/xml");
			st = getChannelAdvertise(index, page);
			out.print(st);
			break;
		}
		case 100:
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case 4: { // add channel
			...
			String name = "channel_video_" + getFileNameByDateTime() + filetype;
			...
			if (temp) {
				...
				if (flag) {
					...
					transferImageFLC.transferImageFLC23(path2, config._pathliveTV);
					out.print("success");
				} else {
					out.print("unsuccess");
				}
...
//		String chuoi = "(" + str + ")";
		...
		for (int i = 0; i < arr.length; i++) {
			filepath = LiveTvServiceDBI.deleteChannelAdvertise(arr[i]);
	...
	public String getChannelAdvertise(int index, int page) {
		ConfigLoader loader = new ConfigLoader();
		Config config = loader.getConfig();
		int c = 0;
		int fr = index * page;
		...
		c = fr;
		int to = (index + 1) * page;
		Vector<LiveTV> vLiveTV = liveTvServiceDBI.getChannelAdvertise(fr, to);
		String mData = "";
		int count = vLiveTV.size();
		mData += "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\" ?>\n";
		mData += "<xml  count=\"" + count + "\" >";
		// if (count < c && count != 0) {
		// if (count % 6 == 0) {
