...
public class Uploadfile extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// cach 2:
		/*
		 * System.out.println("request=" + request); boolean isMultipart =
		 * ServletFileUpload.isMultipartContent(request); if (isMultipart) {
		 * System.out.println("cach 2 save file image"); FileItemFactory factory
		 * = new DiskFileItemFactory(); ServletFileUpload upload = new
		 * ServletFileUpload(factory); try { List items =
		 * upload.parseRequest(request); Iterator iterator = items.iterator();
		 * while (iterator.hasNext()) { FileItem item = (FileItem)
		 * iterator.next(); if (!item.isFormField()) { String fileName =
		 * item.getName(); ImageName +=
		 * fileName.substring(fileName.indexOf("."));
		 * System.out.println("luu file: " + path + ImageName); File
		 * uploadedFile = new File(path + ImageName); try {
		 * item.write(uploadedFile);
		 * System.out.println("luu file thanh cong theo cach 2"); } catch
		 * (Exception ex) { ex.printStackTrace(); } } } } catch (Exception e) {
		 * e.printStackTrace(); } }
		 */
