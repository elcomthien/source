...
public class LiveTVDao {
	...
	public int addChannelAdvertise(LiveTVChannelModel livetv) {
		...
		LogUtil.logDao(LiveTVDao.class.toString(), SQL.ADD_CHANNEL_ADVERTISE, params, "name,link,image,status,index", rs);
		return rs;
	}
	...
	public int editChannelAdvertise(LiveTVChannelModel livetv) {
		...
		LogUtil.logDao(LiveTVDao.class.toString(), SQL.ADD_CHANNEL_ADVERTISE, params, "channelId,name,link,image,status,index", rs);
		return rs;
	}
	...
	@SuppressWarnings("unchecked")
	public List<LiveTVChannelModel> getListAllChannel() {
		List<LiveTVChannelModel> list = new ArrayList<LiveTVChannelModel>();
		Vector<SubProParam> params = new Vector<SubProParam>();
		Vector<String> outParam = new Vector<String>();
		SubProParam subOut = new SubProParam(outParam, "STRING_ARR", 1);
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.GET_LIST_ALL_CHANNEL, params);
			if ((params != null) & (params.size() > 0)) {
				subOut = (SubProParam) params.get(0);
				outParam = subOut.getVector();
			}
		...
		LogUtil.logDao(LiveTVDao.class.toString(), SQL.GET_LIST_ALL_CHANNEL, params, "idSubject", outParam.size() / 9);
		for (int i = 0; i < outParam.size(); i = i + 9) {
			LiveTVChannelModel ch = new LiveTVChannelModel();
			ch.setIdChannel(outParam.get(i));
			ch.setName(outParam.get(i + 1));
	...
	public static void main(String[] args) {
		...
		LiveTVChannelModel li = new LiveTVChannelModel();
		li.setName("Test");
		li.setLink("video_1483093176071.mp4");
		li.setImage("image_1483093176101.png");
		li.setStatus("1");
		li.setCode("0");
//		System.out.println(l.addChannelAdvertise(li));
		System.out.println(l.getListAllChannel());
