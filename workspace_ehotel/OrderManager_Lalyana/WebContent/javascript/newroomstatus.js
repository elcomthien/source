function writeTableRoomStatus(data) {
	var temp = 0;
	var no = 0;
	var text = "";
	var body = "";
...
//						if (value.status == "0") {
							...
							var name = "<td style='text-align: center;' class='fontsizetr'>"
									+ value.statusname + "</td>";
							var date = "<td style='text-align: center;' class='fontsizetr'>"
									+ value.date + "</td>";
//							var timewakeup = "<td style='text-align: center;' class='fontsizetr'>"
//									+ value.hours + ":" + value.minutes + "</td>";
							var action = "<td style='text-align: center;' class='fontsizetr'><a class='cssconfirm' style='cursor: pointer;' onclick=confirmRoomStatus(" + value.folionum + ")>Confirm</a>";
							var status = "";
							text += "<tr style='background-color: #FF6347;'>"
									+ checkdelete + stt + service + folio
									+ name + date + action + status
									+ "</tr>";
//						}
	...
	var table = "<table id='tablelistorder' class='table'><thead>" + "<tr>"
			...
			+ "<th style='text-align: center; width: 18%;'>Implementation</th>"
...
function confirmRoomStatus(folionum){
	$.get("order?action=confirmrs&folionum=" + folionum, function(
			data) {
		writeTableRoomStatus(data);
	});
}

function deleteRoomStatus(id, folionum){
	$.get("order?action=deleters&id=" + id + "&folionum=" + folionum, function(
			data) {
		writeTableRoomStatus(data);
	});
}