...
public class PMSFolioDao {
	...
	public List<PMSFolioRoomModel> getListFolio() {
		...
		LogUtil.logDao(PMSFolioDao.class.toString(), SQL.GET_LIST_FOLIO,
				params, "none", outParam.size() / 3);
	...
	public List<PMSFolioGuestModel> getListGuestForFolio(int folionum) {
		...
		LogUtil.logDao(PMSFolioDao.class.toString(), SQL.GET_LIST_GUEST,
				params, "folionum", outParam.size() / 10);
	...
	public int addOrUpdateGuest(PMSFolioGuestModel guest) {
		...
		LogUtil.logDao(PMSFolioDao.class.toString(), SQL.ADD_OR_UPDATE_GUEST,
				params,
				"folionum,clientid,firstname,lastname,clientname,roomsharer",
				rs);
		return rs;
	}
	...
	public int deleteGuest(String folionum, String clientid) {
		...
		LogUtil.logDao(PMSFolioDao.class.toString(), SQL.DELETE_GUEST, params,
				"folionum,clientid", rs);
		return rs;
	}

	@SuppressWarnings("unchecked")
	public List<PMSFolioMessageModel> getListMessageForRoom(String folioNum,
			String langId) {
		...
		LogUtil.logDao(PMSFolioDao.class.toString(), SQL.GET_LIST_MESSAGE,
				params, "folioNum,langId", outParam.size() / 10);
		for (int i = 0; i < outParam.size(); i = i + 10) {
	...
	public int addMessageFolio(PMSFolioMessageModel mess) {
		...
		LogUtil.logDao(PMSFolioDao.class.toString(), SQL.ADD_MESSAGE_FOLIO,
				params, "folionum,title,content,sender", rs);
		return rs;
	}
	...
	public int deleteMessage(int folionum, int messId) {
		...
		LogUtil.logDao(PMSFolioDao.class.toString(), SQL.DELETE_MESSAGE_FOLIO,
				params, "folionum,messageId", rs);
		return rs;
	}
	...
	public List<HashMap<String, String>> getSmartcardFolio(String folionum) {
		...
		LogUtil.logDao(PMSFolioDao.class.toString(), SQL.GET_SMARTCARD_FOLIO,
				params, "folioNum", outParam.size() / 2);
	...
	public int deleteSmartcard(String serinumber) {
		...
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.DELETE_SMARTCARD_FOLIO,
					params);
			if ((params != null) & (params.size() > 0)) {
		...
		LogUtil.logDao(PMSFolioDao.class.toString(),
				SQL.DELETE_SMARTCARD_FOLIO, params, "serinumber", rs);
		return rs;
	}
	...
	public List<HashMap<String, String>> getListSmartcard() {
		...
		LogUtil.logDao(PMSFolioDao.class.toString(), SQL.GET_LIST_SMARTCARD,
				params, "none", outParam.size() / 3);
		for (int i = 0; i < outParam.size(); i = i + 3) {
			HashMap<String, String> hmap = new HashMap<String, String>();
			hmap.put("serinumber", outParam.get(i));
