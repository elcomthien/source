...
import ehotel.admin.dbi.SystemServiceDBI;
...
public class Background extends ServiceParent {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case 4: {
			...
			this.showJSPpage(request, response, "/system/detailBackgroundImage.jsp");
			break;
		}
		case 5: {
			System.out.println("Get background music for folio type");
			this.showJSPpage(request, response, "/system/detailBackgroundMusic.jsp");
			break;
		}
		default: {
			break;
	...
	public String getFileBackgroundLogo(String link) {
		...
		String[] listCmd = new String[] { "/bin/bash", "-c", "ls " + path + "/bg.* " + path + "/logo*" };
	...
	public UUID transferBackgroundVideo(String filename, String path) {
		...
		String pathsave = config._FilePathserver;
		// ////////////ten file background day len server
		// String namesave = "-100";

		String filetype = "";
		filetype = filename.substring(filename.lastIndexOf("."));
		String namesave = getDefaultName(filetype);
		String hostsrc = "";
		int portsrc = 21;
		String usersrc = "";
		String passsrc = "";
		...
		try {
			...
			id = ftpgateway.download(server, hostserver, pathsave + "/" + namesave, 5000);

		} catch (Exception e) {
			e.printStackTrace();
		}
		if (id != null)
			SystemServiceDBI.updateBGVideo(namesave);
		return id;
	}
	...
	public static void main(String[] args) {
		...
		Date today = Calendar.getInstance().getTime();
		String name = df.format(today);
		System.out.println("Video_bg_" + name);
	}
}
