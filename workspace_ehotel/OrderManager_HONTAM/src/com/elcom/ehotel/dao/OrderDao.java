...
public class OrderDao {
	...
	public List<DiningModel> getNewOrder() {
		String sql = "select ID,DICH_VU,TEN_MON_AN,SO_LUONG,THUOC_PHONG,TO_CHAR(TO_DATE(NGAY_DAT_ORDER,'DD/MM/YYYY'), 'DD-Mon-YY') as NGAY_DAT_ORDER,TRANG_THAI,GIA,CHECK_OUT,TO_CHAR(NGAY_THUC_HIEN, 'DD-Mon-YY HH:MI:SS AM') as NGAY_THUC_HIEN from nhan_order_tu_tivi where xoa = 0 and DICH_VU = 'DINING' order by ngay_thuc_hien desc";
		...
		try {
			...
			if (vector.size() > 2) {
				...
				for (ResultDB result = new ResultDB(vector); result.hasNext(); list.add(order)) {
					...
					order.setMaorder(Integer.parseInt(result.getParam("ID")));
					order.setDichvu(result.getParam("DICH_VU"));
					order.setTenmonan(result.getParam("TEN_MON_AN"));
					order.setSoluong(Integer.parseInt(result.getParam("SO_LUONG")));
					order.setPhong(Integer.parseInt(result.getParam("THUOC_PHONG")));
					order.setNgaydat(result.getParam("NGAY_DAT_ORDER"));
					order.setTrangthai(Integer.parseInt(result.getParam("TRANG_THAI")));
					order.setGia(result.getParam("GIA"));
					order.setCheckout(Integer.parseInt(result.getParam("CHECK_OUT")));
	...
	public static void main(String args[]) {
		...
		System.out.println(orderDao.getListRoomStatus());
	}
