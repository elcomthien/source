...
//--------------------------------------------------------------------------------------
	...
	public String  getAllNorSubjects(String keystb)
	  {		
		  String result_vRS = "";
		  Vector<SubProParam> params = new Vector<SubProParam>(2);
		  
		  SubProParam inSub = new SubProParam(new String(keystb), SubProParam.IN);
	      params.add(inSub);
...
//------------------------------------------------------------------------------
	  @SuppressWarnings("unchecked")
	public String getUrlSub(int ctnid)
	  {
		  String result_vRS = "";		  
		  Vector vRs = new Vector();
	...
	//------------------------------------------------------------------------------
	  ...
	  //----------------------------------------------------------------------------
	//------------------------------------------------------------------------------
	  public String getLinkWelcome(String keystb,String key)
		      
		  {
		    ...
		    //Executes the DB stored procedure
		    params = broker.executeSubPro(SQL.sqlgetLinkWelcome, params);
		    //Get data returned by the stored procedure
		    SubProParam paramOUT = (SubProParam)params.get(0);
		    if (paramOUT.getString() == null)
		    	welcomelink = "";
		    else welcomelink = (paramOUT.getString());
