...
public class CasContent {
	...
	public String getNameFolio(String folionumber)   
	{
		String retur = "no room";
		Vector params = new Vector(2);
		
		SubProParam out_data = new SubProParam(new String(), SubProParam.OUT);
	    params.add(out_data);
	    
		SubProParam  folionumber_ = new SubProParam(new String(folionumber),SubProParam.IN);
	    params.add(folionumber_);
	    
	    params = broker.executeSubPro(SQL.sqlgetNameFolio,params);
	    out_data = (SubProParam) params.get(0);
		retur = out_data.getString();
		return retur;
	}
	 ...
	 * @return: StbVO
	 */
	public String login(String keystb,String ip) 
	{
		String xml  = "";
		...
		/*String roomname;
		if ( new Integer(stbvo.getFolio()).intValue() < 10) {
			 System.out.println("ROOM NUMBER:" + "00" + stbvo.getFolio());
			 roomname = getNameFolio("00" + stbvo.getFolio());
		}else
		if ( new Integer(stbvo.getFolio()).intValue() < 100) {
			System.out.println("ROOM NUMBER:" + "0" + stbvo.getFolio());
			 roomname = getNameFolio("0" + stbvo.getFolio());
		}else
		{	
			System.out.println("ROOM NUMBER:" + stbvo.getFolio());
			roomname = getNameFolio(stbvo.getFolio());
		} */
		xml = "<?xml version='1.0' encoding='UTF-8'?>\r\n<Stb>\r\n";
		xml = xml + "<item>";
		xml = xml + "<key>" + stbvo.getKeystb() + "</key>\r\n";
		xml = xml + "<room>" + stbvo.getFolio() + "</room>\r\n";
		//xml = xml + "<roomname>" + roomname + "</roomname>\r\n";
		xml = xml + "</item>";
		xml = xml + "</Stb>";
		return xml;
	}
...
//---------------------------------------------------------------------------------
	...
	public void updateIPbox(String keystb,String ip)
	{
		Vector params = new Vector(3);
	    ...
	    broker.executeSubPro(SQL.sqlupdateIPbox,params);
	    out_data = (SubProParam) params.get(0);
		String Rs = out_data.getString();
	}
	
	
	public String getLangs(String keystb)
	{
		String Rs = "";
		Vector params = new Vector(2);
		
		if (keystb == null ) keystb = "2001";
		SubProParam out_data = new SubProParam(new String(), SubProParam.OUT);
	    params.add(out_data);
	...
	public String getHelp(String sernum)
	{
		String Rs = "";
		Vector params = new Vector(2);
	    ...
	    params = broker.executeSubPro(SQL.sqlgethelp,params);
	    out_data = (SubProParam) params.get(0);
		Rs = out_data.getString();
	...
	public String getConfigInfo(String key)
	{
		...
		SubProParam  key_ = new SubProParam(new String(key),SubProParam.IN);
	    params.add(key_);
	    
		
	    params = broker.executeSubPro(SQL.sqlgetConfigInfo,params);
	    out_data = (SubProParam) params.get(0);
		Rs = out_data.getString();
