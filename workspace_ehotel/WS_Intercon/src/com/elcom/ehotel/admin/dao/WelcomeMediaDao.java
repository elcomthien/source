...
public class WelcomeMediaDao {
	...
	public List<WelcomeMediaModel> getWelcomeMediaGroup(String type, String idgroup) {
		...
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.GET_WELCOME_MEDIA_GROUP, params);
			if ((params != null) & (params.size() > 0)) {
		...
		LogUtil.logDao(WelcomeMediaDao.class.toString(), SQL.GET_WELCOME_MEDIA_GROUP, params, "type,idgroup", outParam.size() / 5);
		for (int i = 0; i < outParam.size(); i = i + 5) {
			WelcomeMediaModel wlc = new WelcomeMediaModel();
			wlc.setId(outParam.get(i));
			wlc.setName(UnicodeConverter.decodeUnicode(outParam.get(i + 1)));
	...
	public int addWelcomeMediaGroup(WelcomeMediaModel wlc) {
		...
		in = new SubProParam(new String(wlc.getType()), 0);
		params.add(in);
		in = new SubProParam(new String(wlc.getIdgroup()), 0);
		params.add(in);

		SubProParam subOut = new SubProParam(new String(), SubProParam.OUT);
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.ADD_WELCOME_MEDIA_GROUP, params);
			if ((params != null) & (params.size() > 0)) {
		...
		LogUtil.logDao(WelcomeMediaDao.class.toString(), SQL.ADD_WELCOME_MEDIA_GROUP, params, "name,filename,type,idgroup", rs);
		return rs;
	}
	...
	public int editWelcomeMediaGroup(WelcomeMediaModel wlc) {
		...
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.EDIT_WELCOME_MEDIA_GROUP, params);
			if ((params != null) & (params.size() > 0)) {
		...
		LogUtil.logDao(WelcomeMediaDao.class.toString(), SQL.EDIT_WELCOME_MEDIA_GROUP, params, "id,name,filename,index,status,idgroup", rs);
		return rs;
	}
	...
	public int deleteWelcomeMediaGroup(String id, String idgroup) {
		...
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.DELETE_WELCOME_MEDIA_GROUP, params);
			if ((params != null) & (params.size() > 0)) {
		...
		LogUtil.logDao(WelcomeMediaDao.class.toString(), SQL.DELETE_WELCOME_MEDIA_GROUP, params, "id,idgroup", rs);
		return rs;
	}
}
