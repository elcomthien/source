...
public class VodDao {
	...
	public List<VodSubjectModel> getListSubjectVod(int langId, String type) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.GET_LIST_VOD_SUBJECT, params, "langid,type", outParam.size() / 7);
	...
	public int addVodSubject(VodSubjectModel vod) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.ADD_VOD_SUBJECT, params, "name,image,invisible,type", rs);
		return rs;
	}
	...
	public int editVodSubject(VodSubjectModel vod) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.EDIT_VOD_SUBJECT, params, "idsubject,name,image,invisible,type", rs);
		return rs;
	}
	...
	public List<VodContentModel> getListContent(VodContentModel vod) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.GET_LIST_VOD_CONTENT, params, "idsubject,langid", outParam.size() / 16);
		for (int i = 0; i < outParam.size(); i = i + 16) {
			...
			con.setIndex(outParam.get(i + 15));
			list.add(con);
		}
		return list;
	}
	...
	public int addNewMovie(VodContentModel con) {
		...
		params.add(in);
		in = new SubProParam(new BigDecimal(con.getIndex()), 0);
		params.add(in);
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.ADD_VOD_CONTENT, params,
				"idsubject,name,production,director,actor,poster,plot,price,iunit,url,invisible,isnew,index", rs);
		return rs;
	}
	...
	public int editMovie(VodContentModel con) {
		...
		LogUtil.logDao(VodDao.class.toString(), SQL.EDIT_VOD_CONTENT, params,
				"idsubject,idcontent,name,production,director,actor,poster,plot,price,iunit,invisible,isnew,langid,index", rs);
		return rs;
	}
	...
	public static void main(String[] args) {
		VodDao v = new VodDao();
		System.out.println(v.getSubtitle(1608));
	}
}
