...
public class FolioServiceDBI {
	...
	public static final String EDIT_GUEST_NAME = "BEGIN EPMS.editGuestName(?,?,?); END;";
	public static final String REMOVE_BILL_ID = "BEGIN EPMS.remove_bill_id(?); END;";
	public static final String UPDATE_MESSAGE_DETAIL = "BEGIN EPMS.UPDATE_MESSAGE_DETAIL(?,?,?,?,?,?); END;";
	...
	public boolean insertMessage(int id, String subject, String content,
		...
		param.add(in3);// 3
		try {
			System.out.println("111111111111111111111111111111111111111: "+id+" id: "+param.get(0).getBigDecimal()+" subject: "+param.get(1).getString()+" content: "+param.get(2).getString()+" content: "+param.get(3).getString());
	...
	@SuppressWarnings("rawtypes")
	public List<GuestModel> getGuestForFolio(String id) {
		List<GuestModel> list = new ArrayList<GuestModel>();
		String sql = "select * from pms_guestprofile where folionum = " + id
				+ " and client_status = 1 order by CLIENT_ID asc";
		Vector vector = new Vector();
		try {
			vector = broker.executeSelect(sql, null);
			if (vector.size() > 2) {
				...
				while (result.hasNext()) {
					...
					guest.setClientid(result.getParam("CLIENT_ID"));
					...
					guest.setClientstatus(result.getParam("CLIENT_STATUS"));
					guest.setRoomshare(result.getParam("ROOM_SHARER"));
					guest.setPincode(result.getParam("PINCODE"));
					list.add(guest);
				}
			}
		} catch (Exception e) {
	...
	public boolean editGuestName(int room, int id, String name){
		boolean flag = true;
		Vector<SubProParam> param = new Vector<SubProParam>(2);
		SubProParam in = new SubProParam(new BigDecimal(room), SubProParam.IN);
		param.add(in);// 0
		in = new SubProParam(new BigDecimal(id), SubProParam.IN);
		param.add(in);// 1
		in = new SubProParam(new String(name), SubProParam.IN);
		param.add(in);
		try {
			broker.executeSubPro(EDIT_GUEST_NAME, param);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
	...
	@SuppressWarnings("rawtypes")
	public boolean remove_Bill_ID(int id) {
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(id),
				SubProParam.IN);
		params.add(in);
		try {
			broker.executeSubPro(REMOVE_BILL_ID, params);
		} catch (Exception e) {
			e.printStackTrace();
			flag = false;
		}
	...
	@SuppressWarnings("rawtypes")
	public List<EPMS_BILL> getGUESTPROFILE(int id) {
		List<EPMS_BILL> list = new ArrayList<EPMS_BILL>();
		//String sql = "select smart_card from pms_foliostb where folio_code = '"+ id + "' order by smart_card asc";
		String sql="select folionum,client_id from pms_guestprofile where folionum="+id+" and client_status=1 and firstname <>  'ROOM'";
		Vector vector = new Vector();
		try {
			vector = broker.executeSelect(sql, null);
			if (vector.size() > 2) {
				...
				while (result.hasNext()) {
					result.next();
					EPMS_BILL eb= new EPMS_BILL();
					eb.setFolionum(result.getParam("folionum"));
					eb.setClient_id(result.getParam("client_id"));
					list.add(eb);
					System.out.println("-------getGUESTPROFILE-------.size(): "+list.size());
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public Message_Detail getListMessage_Detail(int id_room,int id_message,int lang_id) {
		Message_Detail eb= new Message_Detail();
		//String sql = "select smart_card from pms_foliostb where folio_code = '"+ id + "' order by smart_card asc";
		String sql="select MESSAGE_ID,FOLIONUM,SUBJECT,CONTENT,SENDER from PMS_FOLIOMESSAGE where MESSAGE_ID="+id_message+" and FOLIONUM="+id_room+" and LANG_ID="+lang_id+"";
		Vector vector = new Vector();
		try {
			vector = broker.executeSelect(sql, null);
			if (vector.size() > 2) {
				...
				while (result.hasNext()) {
					result.next();					
					eb.setId_message(result.getParam("MESSAGE_ID"));
					eb.setId_room(result.getParam("FOLIONUM"));
					eb.setSubject(result.getParam("SUBJECT"));
					eb.setContent(result.getParam("CONTENT"));
					eb.setSender(result.getParam("SENDER"));					
	...
	public boolean UpdateMessage_Detail(int id_room,int id_message,int lang_id,String subject,String sender,String content) {
		//System.out.println("id_room: "+id_room+ " id_message: "+id_message+" lang_id: "+lang_id+ " subject: "+subject+" sender: "+sender+" content: "+content);
		boolean flag = true;
		Vector<SubProParam> param = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(id_room), SubProParam.IN);
		param.add(in);// 0
		in = new SubProParam(new BigDecimal(id_message), SubProParam.IN);
		param.add(in);// 1
		in = new SubProParam(new BigDecimal(lang_id), SubProParam.IN);
		param.add(in);// 1
		in = new SubProParam(new String(subject), SubProParam.IN);
		param.add(in);
		in = new SubProParam(new String(sender), SubProParam.IN);
		param.add(in);
		in = new SubProParam(new String(content), SubProParam.IN);
		param.add(in);
		try {
			broker.executeSubPro(UPDATE_MESSAGE_DETAIL, param);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
	...
	@SuppressWarnings("rawtypes")
	 public static String getURLPromotion(int id, int lang) {
		//System.out.println("pms_promotion: id: "+id+" lang: "+lang);
	  String link = "";
