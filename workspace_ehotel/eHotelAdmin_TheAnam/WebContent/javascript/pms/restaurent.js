...
function AddRestaurant() {
	...
	this.show = function() {
		...
		f.complet = function(tx) {
			...
			result += hostname;
			document.getElementById("iframe_hotel").src = result
					+ ":8383/WebEditor/hello.elcom#" + my.subid;
...
function editRestaurant() {
	...
	this.show = function() {
		var id_retaurent = my.itemId;

		var url = "ServiceDinning?CMD=13";
		url += "&id=" + id_retaurent;
		url += "&r=" + Math.random();
		...
		f.complet = function(tx) {
			...
			var it = tx.getElementsByTagName("Item");
			for ( var i = 0; i < it.length; i++) {
				des_Value = it[i].getElementsByTagName("des")[0].childNodes[0].nodeValue;
				if (des_Value != null) {
					layer.show();
					var html = "";
					var url = "ServiceDinning?CMD=7";
					...
					f.complet = function(tx) {
						...
						var hostname = window.location.hostname;
						result += hostname;
						var index = des_Value.lastIndexOf("/");
						name_file = des_Value.substring(index + 1);
						$("form_textbox_cancel").onclick = my.cancel;
						$("form_textbox_close").onclick = my.cancel;
						document.getElementById("iframe_hotel").src = result
								+ ":8383/WebEditor/excute.elcom#" + name_file
								+ "@" + my.obj.Title + "&" + my.obj.Id;
					};
				}
			}
		};
...
/*
 * function get_des_retaurent(id_retaurent_){ var des_Value=""; var
 * url="ServiceDinning?CMD=13" url+="&id="+id_retaurent_;
 * url+="&r="+Math.random(); var f = new AjaxGetXML(url); f.complet =
 * function(tx) { var xml=tx.getElementsByTagName("xml"); var
