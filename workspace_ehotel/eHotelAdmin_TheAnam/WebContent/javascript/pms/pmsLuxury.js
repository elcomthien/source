...
function listHotel() {
	...
	ctTable.change = function() {
		var obj = this.obj;
		var url1 = "Luxury?CMD=6";
		url1 += "&id=" + obj.Id;
		url1 += "&r=" + Math.random();
		var obj = this.obj;
		var url2 = "Luxury?CMD=5";
		url2 += "&id=" + obj.Id;
		url2 += "&r=" + Math.random();
		var url3 = "Luxury?CMD=7";
		url3 += "&id=" + obj.Id;
	...
	table.dataBind = function() {
		...
		for ( var i = 0; i < this.List.length; i++) {
			...
			html += "</td>";
			html += "<td align=\"left\"  valign=\"middle\" title='" + this.List[i].Def + "'>";
			html += (des);
			html += "</td>";
	...
	this.get = function(index) {
		...
		f.complet = function(tx) {
			...
			for ( var i = 0; i < it.length; i++) {
				...
				arr[i] = new ItemLuxury(_id, _name, _def, _type);
			}
			table.load(arr);
...
function ItemLuxury(id, name, def, type) {
	this.Id = id;
	this.Name = name;
	this.Def = def;
	this.Type = type;
...
function detailItem() {
	...
	this.show = function() {
		...
		f.complet = function(tx) {
			...
			result+=hostname;
			var string =my.obj.Des;
			var index = string.lastIndexOf("/");
			name_file= string.substring(index+1);			
			$("form_textbox_cancel").onclick = my.cancel;	
			$("form_textbox_close").onclick = my.cancel;
			document.getElementById("iframe_hotel").src = result+":8383/WebEditor/excute.elcom#"+name_file+"@"+my.obj.Title +"&" +my.obj.Id;
...
function addItemHotel() {
	...
	this.show = function() {
		...
		f.complet = function(tx) {
			...
			var hostname=window.location.hostname;
			result+=hostname;
			document.getElementById("iframe_hotel").src = result+":8383/WebEditor/hello.elcom#"+my.subid;
			$("form_textbox_cancel").onclick = my.cancel;
			$("form_textbox_close").onclick = my.cancel;
			/*$("form_textbox_ok").onclick = function() {
				my.accept();
...
//		html += "</li>";
	...
	this.renderClick = function() {
		var ul = $("div_contextMenu").getElementsByTagName("ul");
		var li = ul[0].getElementsByTagName("li");
		var addVideo = li[0].getElementsByTagName("a")[0];
		addVideo.onclick = function() {
			my.addVideo();
		};
		var addText = li[1].getElementsByTagName("a")[0];
		addText.onclick = function() {
			my.addText();
		};
		var itemdetail = li[2].getElementsByTagName("a")[0];
		itemdetail.onclick = function() {
			my.fucDetail();
		};
		var itemdel = li[3].getElementsByTagName("a")[0];
		itemdel.onclick = function() {
			my.fucdel();
		};
