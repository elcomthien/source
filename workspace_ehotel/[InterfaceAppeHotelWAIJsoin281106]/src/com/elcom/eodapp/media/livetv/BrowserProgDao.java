...
public class BrowserProgDao {
	...
	public String getListChannel() throws IOException {
		String jstext = "";
		JSONObject obj = new JSONObject();
		JSONArray ja = new JSONArray();
		...
		params.add(subOut);
		try {
			params = broker.executeSubPro(SQL.GET_ALL_CHANNEL, params);
	...
	@SuppressWarnings({ "unchecked" })
	public String getListChannelAdvertise() throws IOException {
		String jstext = "";
		JSONObject obj = new JSONObject();
		JSONArray ja = new JSONArray();
		Vector<SubProParam> params = new Vector<SubProParam>();
		...
		params.add(subOut);
		try {
			params = broker.executeSubPro(SQL.GET_CHANNEL_ADVERTISE, params);
			if ((params != null) & (params.size() > 0)) {
				subOut = (SubProParam) params.get(0);
				outParam = subOut.getVector();
			}
	...
	public static void main(String[] args) throws IOException {
		BrowserProgDao b = new BrowserProgDao();
		System.out.println(b.getListChannel());
	}
