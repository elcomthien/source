...
public class ModDao {
	...
	public List<ModSubjectModel> getListSubjectMod(int langid) {
		...
		for (int i = 0; i < outParam.size(); i = i + 5) {
			...
			mod.setName(UnicodeConverter.decodeUnicode(outParam.get(i + 1)));
	...
	public int addSubjectMod(ModSubjectModel sub) {
		...
		LogUtil.logDao(ModDao.class.toString(), SQL.ADD_MOD_SUBJECT, params, "name,image,invisible", rs);
		return rs;
	}
	...
	public int editSubjectMod(ModSubjectModel sub) {
		...
		LogUtil.logDao(ModDao.class.toString(), SQL.EDIT_MOD_SUBJECT, params, "idSubject,name,image,invisible,langid", rs);
		return rs;
	}
