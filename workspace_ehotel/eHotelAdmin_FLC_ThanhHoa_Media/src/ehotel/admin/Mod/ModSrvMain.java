	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case 2:// xoa subject
		{
			...
			if (request.getParameter("id") != null) {
				...
				modContentServiceDBI.removeSubject(idsub, "SERVICE");
		...
		case 4:// add mod to service
		{
			...
			modContentServiceDBI.addServiceMod(subid, chuoi);
			out.print(t);
		...
		case 5:// delete service MOD
		{
			...
			if (iuser.isAdmin(ipAdress)
				...
				modContentServiceDBI.removeServiceMod(subid, chuoi);
			} else {
				out.print(-1);
			}
			break;
		...
		case 8://
		{
			...
			modContentServiceDBI.changeSubjectOfMod(modId, param, "Service");
		}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case 1:// /insert subject
		{
			...
			if (request.getParameter("name") != null) {
				...
				modContentServiceDBI.addNewSubject(subjectName, image, urlBg, parent, "SERVICE");
				if (id > 0) {
					...
					transferImageFLC.transferImageMain(path1, config._music);
					...
					transferImageFLC.transferImageMain(path3, config._music);
		...
		case 2:// edit subject
		{
			...
			if (request.getParameter("name") != null) {
				if (request.getParameter("subid") != null) {
					...
					modContentServiceDBI.editSubject(subjId, subjectName, image, urlBg, LangID, "SERVICE");
					out.print(t);
					if (t > 0) {
						// luu hinh anh subject
						ManagerFile file = new ManagerFile();
						...
						file.copy(path1, path2);
						transferImageFLC
								.transferImageMain(path1, config._music);
						file.deletefile(path1);
						// luu hinh anh cua subject background
						String path3 = config._temp + urlBg;
						String path4 = config._pathImage + config._music + "/"
						...
						file.copy(path3, path4);
						transferImageFLC
								.transferImageMain(path3, config._music);
						file.deletefile(path3);
					}
				}
			}
		...
		case 3:// update song
		{
			...
			modContentServiceDBI.updateMod(id, title, singer, def, title, album, LangID, "SERVICE");
			out.print(t);
			break;
		}
		default:
