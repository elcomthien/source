...
public class serviceVod extends ServiceParent {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case 1: {
			...
			String st = getVodSubjectService();
			out.print(st);
			break;
		}
		case 2: {
		...
		case 5:// new released
		{
			...
			vodContentServiceDBI.setNewRelease(vodId, subId);
			break;
		}
		case 6:// status
		{
			...
			vodContentServiceDBI.setVisileStatus(vodId, subId);
			break;
		}
		case 7:// change subject
		{
			...
			vodContentServiceDBI.changeSubjectOfSvcVod(vodId, param);
			break;
		}
		case 8:// list vod in Subcontent not SubService
		{
			...
			if (request.getParameter("subISvr") != null) {
				subISvr = Integer.parseInt(request.getParameter("subISvr").toString());
			}
		...
		case 9:// add contènt srvice
		{
			...
			vodContentServiceDBI.addVodToService(subId, str);
		...
		case 10:// remove
		{
			...
			if (iuser.isAdmin(ipAdress) || iuser.checkRoleUser(ipAdress, Def._roleVIDEO)) {
				...
				vodContentServiceDBI.removeVodService(subId, chuoi);
			} else {
				out.print(-1);
			}
			break;
		...
		case 11:// add content service
		{
			...
			vodContentServiceDBI.addVodToService(subId, chuoi);
			if (b) {
				out.write("1");
			} else {
				out.write("0");
		...
		case 12:// change status vod subject service
		{
			int subId = -1;
			int invisible = -1;
			if (request.getParameter("subid") != null) {
			...
			boolean flag = true;
			if(invisible == 0)
				flag = vodContentServiceDBI.updateInvisibleVodSubjectService(subId, 1);
			else if(invisible == 1)
				flag = vodContentServiceDBI.updateInvisibleVodSubjectService(subId, 0);
			if(flag)
				out.write("t");
			else
				out.write("f");
		...
		case 13:// search movie service
		{
			...
			response.setContentType("text/xml");
			String st = searchVodSrv(subId, index, page, text);
//			System.out.println(st);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case 1: {
			...
			if (request.getParameter("name") != null) {
				...
				VOD vodDBI = DBIGateway.getAMDVod();
				AMDVod srvMod = vodDBI.getAMDSvcVod();
				// int id= srvMod.addSubject(subjectName, config._movie + "/" + image,-1);
				int id = srvMod.addSubject(subjectName, image, -1);
				vodContentServiceDBI.addSubject(subjectName, image, -1, "SERVICE");
				if (id > 0) {
		...
		case 2:// update subject vod service subject 18.1
		{
			...
			if (request.getParameter("name") != null) {
				if (request.getParameter("subid") != null) {
					...
					VOD vodDBI = DBIGateway.getAMDVod();
					AMDVod srvMod = vodDBI.getAMDSvcVod();
					// int t= srvMod.editSubject(subjId,subjectName, config._movie + "/" +image
					// ,LangID);
					int t = srvMod.editSubject(subjId, subjectName, image, LangID);
					vodContentServiceDBI.editSubject(subjId, subjectName, image, "SERVICE", LangID);
					out.print(t);
					if (t > 0) {
						ManagerFile file = new ManagerFile();
						String path1 = config._temp + "/" + image;
		...
		case 3: {
			...
			if (request.getParameter("subid") != null) {
				...
				vodContentServiceDBI.removeSubject(subjId, "SERVICE");
		...
		case 4: {
			...
			if (iuser.isAdmin(ipAdress) || iuser.checkRoleUser(ipAdress, Def._roleVIDEO)) {
				...
				vodContentServiceDBI.updateVod(id, name, director, actors, def, image, currency, unit, "SERVICE");
				if (b) {
					ManagerFile file = new ManagerFile();
					String path1 = config._temp + "/" + image;
					String path2 = config._pathMovies + "/" + image;
	...
	private String getVodSubjectService() {
		List<VodSubjectService> list = new ArrayList<VodSubjectService>();
		list = vodContentServiceDBI.getVodSubjectService(LangID);
		String mData = "";
		mData += "<?xml version=\"1.0\" encoding=\"ISO-8859-1\" standalone=\"yes\" ?>\n";
		mData += "<xml>";
	...
	private String getListMovieCtn(int subIdCtn, int subj_svc) {
		/*
		 * VOD VodDBI = DBIGateway.getAMDVod(); AMDVod CtnMod = VodDBI.getAMDCntVod(); Vector<Vod>
		 * vod=CtnMod.getVods(subIdCtn, LangID, -1, -1);
		 * 
		 * String mData="";
		 * mData+="<?xml version=\"1.0\" encoding=\"ISO-8859-1\" standalone=\"yes\" ?>\n";
		 * mData+="<xml>";
		 * 
		 * for(int i=0;i<vod.size();i++) { String actor="unknow"; if(vod.get(i).getActors()!=null) {
		 * actor=vod.get(i).getActors(); } mData+="<Item>"; mData+="<name>";
		 * mData+="<![CDATA["+vod.get(i).getTitle()+"]]>"; mData+="</name>"; mData+="<id>\n";
		 * mData+=vod.get(i).getId(); mData+="</id>"; mData+="<Actors>";
		 * 
		 * mData+="<![CDATA["+actor+"]]>"; mData+="</Actors>"; mData+="<Director>";
		 * mData+="<![CDATA["+vod.get(i).getDirector()+"]]>"; mData+="</Director>";
		 * mData+="<Duration>"; mData+="00"; mData+="</Duration>"; mData+="<New>";
		 * mData+=vod.get(i).getReleased(); mData+="</New>"; mData+="<Desc>";
		 * 
		 * mData+="<![CDATA["+(vod.get(i).getPlot())+"]]>"; mData+="</Desc>"; mData+="<Image>";
		 * mData+= vod.get(i).getPoster(); mData+="</Image>"; mData+="<Status>";
		 * mData+=vod.get(i).getStatus(); mData+="</Status>"; mData+="</Item>"; } mData+="</xml>";
		 * 
		 * return mData;
		 */
		VOD VodDBI = DBIGateway.getAMDVod();
		AMDVod CtnMod = VodDBI.getAMDSvcVod();
		Vector<Vod> vod = CtnMod.getVascVods(subIdCtn, subj_svc, LangID, -1, -1);
	...
	private String searchVodSrv(int subId, int index, int page, String text) {
		VOD VodDBI = DBIGateway.getAMDVod();
		AMDVod SrvMod = VodDBI.getAMDSvcVod();
		IEVS evs = VodDBI.getEVSVod();
		int fr = index * page;
...
//		}
		int count = SrvMod.countVodOfSubject(subId);
		List<Vod> vod = vodContentServiceDBI.searchVodSrv(subId, text, LangID, fr, to);
		String mData = "";
		mData += "<?xml version=\"1.0\" encoding=\"ISO-8859-1\" standalone=\"yes\" ?>\n";
		mData += "<xml count='" + count + "'>";
		System.out.println("length:" + vod.size());
	...
	public static void main(String[] args) {
		serviceVod s = new serviceVod();
		System.out.println(s.searchVodSrv(30,-1,-1,""));
