...
public class Contentvod extends ServiceParent {
	TransferImageFLC transferImageFLC = new TransferImageFLC();
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case 2:// get movie content
		{
			...
			list = vodContentServiceDBI.getVodTransferPercent();
			if (list.size() > 0) {
		...
		case 5: // change subject content
		{
			...
			vodContentServiceDBI.changeSubjectOfVod(VodId, subId);
		...
		case 11:// delete subtitle
		{
			...
			while (request.getParameter("id" + i) != null) {
				...
				System.out.println("id content vod for delete sub = = = = "
						+ id);
				String filename = vodContentServiceDBI.getSubVodById(id);
		...
		case 17: {
			...
			vodContentServiceDBI.deleteSubVodByUrl(name);
			break;
		}
		case 18: {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case 1: {
			...
			if (request.getParameter("name") != null) {
				...
				int id = ctnMod.addSubject(subjectName, config._movie + "/"
						+ image, parent);
				vodContentServiceDBI.addSubject(subjectName, config._movie
				...
				if (id > 0) {
					...
					transferImageFLC.transferImageMain(path1, config._movie);
		...
		case 2:// update subject
		{
			...
			if (request.getParameter("name") != null) {
				if (request.getParameter(Def.SubId) != null) {
					...
					vodContentServiceDBI.editSubject(subjId, subjectName, config._movie + "/" + image, "CONTENT", LangID);
					out.print(t);
					if (t > 0) {
						ManagerFile file = new ManagerFile();
						String path1 = config._temp + "/" + image;
		...
		case 3: {
			...
			if (request.getParameter(Def.SubId) != null) {
				...
				vodContentServiceDBI.removeSubject(subjId, "CONTENT");
			}
			break;
		}
		case 4: {
		...
		case 5: {
			...
			vod.setIUnit(money);
			addVod(subId, vod);
			String maxid = vodContentServiceDBI.getMaxIdVodContent();
			String filetype = filename.substring(filename.length() - 3,
					filename.length());
			vodContentServiceDBI.updateUrlForVod(Integer.parseInt(maxid), maxid
					+ "." + filetype);
			boolean rs = transferVod(file, maxid + "." + filetype, name,
					hostsrc, portsrc, usersrc, passsrc);
	...
	private void updateVod(HttpServletRequest request,
		...
		vodContentServiceDBI.updateVod(id, name, director, actors, def, image, String.valueOf(price), money, "CONTENT");
		if (b) {
			ConfigLoader loader = new ConfigLoader();
			Config config = loader.getConfig();
			ManagerFile file = new ManagerFile();
		...
		try {
			...
			if ("mp4".equalsIgnoreCase(file) || "mkv".equalsIgnoreCase(file)
				...
				if (id != null) {
					...
					vodContentServiceDBI.insertVodRemote(filenameServer, id,
							nameview);
	...
	public boolean deleteTransferVod(String uuid, String filename) {
		...
		try {
			...
			boolean t = vodContentServiceDBI.deleteVodRemotePercentByUUID(uuid);
	...
	public boolean deleteFileInFtpServer(int VodId) {
		String filename = vodContentServiceDBI.getFileNameContentById(VodId);
		// String ipserver = contentServiceDBI.getHostPortContentById(VodId);
		List<String> listsub = vodContentServiceDBI.getAllSubForVod(VodId);
		boolean flag = false;
