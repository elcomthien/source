...
public class VodContentServiceDBI {
	...
	// //////////////////////DB 2/////////////////////////
	public static final String CHANGE_SUBJECT_VOD = "BEGIN EVOD.changeSubjectOfVod(?,?,?); END;";
	public static final String ADD_SUBJECT = "BEGIN EVOD.addSubject(?,?,?,?,?); END;";
	public static final String EDIT_SUBJECT = "BEGIN EVOD.editSubject(?,?,?,?,?,?); END;";
	public static final String REMOVE_SUBJECT = "BEGIN EVOD.removeSubject(?,?,?); END;";
	public static final String UPDATE_VOD = "BEGIN EVOD.updateVod(?,?,?,?,?,?,?,?,?,?); END;";
	public static final String SET_NEW_RELEASE = "BEGIN EVOD.setNewReleased(?,?,?); END;";
	public static final String SET_INVI_STATUS = "BEGIN EVOD.setVisileStatus(?,?,?); END;";
	public static final String CHANGE_SVC_SUBJECT_VOD = "BEGIN EVOD.changeSvcSubjectOfVod(?,?,?); END;";
	public static final String ADD_VOD_TO_SERVICE = "BEGIN EVOD.addSvcVod(?,?,?); END;";
	public static final String REMOVE_VOD_SERVICE = "BEGIN EVOD.removeSvcVod(?,?,?); END;";
	...
	public boolean deleteSubVodByUrl(String filename) {
		...
		try {
			...
			broker2.executeSubPro(DELETE_SUBTITLE_BY_URL, params);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
	...
	public boolean updateUrlForVod(int id, String url) {
		...
		try {
			...
			broker2.executeSubPro(UPDATE_URL_FOR_VOD, params);
		} catch (Exception e) {
			e.printStackTrace();
			flag = false;
		}
	...
	public boolean deleteVodContentById(int vodId) {
		...
		try {
			...
			broker2.executeSubPro(DELETE_VOD_BY_ID, param);
		} catch (Exception e) {
			e.printStackTrace();
			return false;
		}
	...
	public boolean updateStatusInvisibleVod(int vodId, String filename) {
		...
		try {
			...
			broker2.executeSubPro(UPDATE_STATUS_VOD, params);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
	...
	public void addVodDbi(int subjectid, int storageid, String url_multicast,
		...
		try {
			...
			broker2.executeSubPro(ADD_VOD, params);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public int deleteSubtitleDbi(int contentid) {
		...
		try {
			...
			broker2.executeSubPro(DELETE_SUBTITLE, params);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public int addSubtitleDbi(int contentid, String url, int langid) {
		...
		try {
			...
			broker2.executeSubPro(ADD_SUBTITLE, params);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return seqout;
	...
	public int deleteVodDbi(int sunjectId) {
		...
		try {
			...
			broker2.executeSubPro(DELETE_VOD, params);
	...
	public boolean updateInvisibleVodSubjectService(int id, int invi) {
		...
		try {
			...
			broker2.executeSubPro(UPDATE_VOD_SUBJECT_SERVICE, params);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
	...
	@SuppressWarnings({ "rawtypes" })
	public List<Vod> searchVodCnt(int subjId, String vodName, int langid,
			int from, int tto) {
		...
		String sql = "select cnt,contentname,director,actors,plot,poster,currency,iunit,isnew,invisible,(select count(*) from vod_contenturltrailer where contentid=cnt) as istrailer, (select count(*) from vod_subtitle where contentid=cnt) as issubtitle "
				...
				+ " and lang_id="
				+ langid
				+ " and invisible = 0 and filepath is not null and (lower(contentname) like  '%"
				+ vodName
				+ "%' or upper(contentname) like  '%"
				+ vodName
				+ "%') ) " + "where stt between " + from + " and " + tto;
		Vector vector = new Vector();
		vector = broker.executeSelect(sql, null);
		if (vector.size() > 2) {
			ResultDB result = new ResultDB(vector);
	...
	@SuppressWarnings("rawtypes")
	public List<Vod> searchVodSrv(int subjId, String vodName, int langid,
			int from, int tto) {
		List<Vod> list = new ArrayList<Vod>();
		String sql = "select  cn,contentname ,director,actors,plot,poster,currency,iunit,isnew,id,id_,invisible,(select count(*) from vod_contenturltrailer where contentid=cn) as istrailer, (select count(*) from vod_subtitle where contentid=cn) as issubtitle"
				...
				+ " where cnt.contentid = ss.contentid  and ss.SVC_ID = cnt.SVC_ID and cnt.invisible = 0 and ss.subjectid="
				+ subjId
				+ "  and cnt.lang_id="
				+ langid
				+ " and (lower(contentname) like  '%"
				+ vodName
				+ "%' or upper(contentname) like  '%"
				+ vodName
				+ "%') ) where stt between " + from + " and  " + tto;
	...
	@SuppressWarnings("unchecked")
	public boolean changeSubjectOfVod(int vodid, int newsub) {
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(vodid), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new BigDecimal(newsub), SubProParam.IN);
		params.add(in);

		SubProParam out = new SubProParam(new BigDecimal(-1), SubProParam.OUT);
		params.add(out);
		try {
			params = broker2.executeSubPro(CHANGE_SUBJECT_VOD, params);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
	...
	public boolean addSubject(String subjectname, String image, int parent,
			String type) {
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new String(subjectname),
				SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(image), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new BigDecimal(parent), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(type), SubProParam.IN);
		params.add(in);
	...
	public boolean editSubject(int subid, String subjectname, String image,
			String type, int langid) {
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(subid), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(subjectname), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(image), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new BigDecimal(langid), SubProParam.IN);
	...
	public boolean removeSubject(int subid, String type) {
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(subid), SubProParam.IN);
		params.add(in);
	...
	public boolean updateVod(int vodid, String title, String director,
			String actor, String plot, String poster, String currency,
			String iunit, String type) {
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(vodid), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(title), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(director), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(actor), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(plot), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(poster), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(currency), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(iunit), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(type), SubProParam.IN);
		params.add(in);
		...
		params.add(out);
		try {
			broker2.executeSubPro(UPDATE_VOD, params);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
	...
	public boolean setNewRelease(int vodid, int subid){
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(vodid), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new BigDecimal(subid), SubProParam.IN);
		params.add(in);

		SubProParam out = new SubProParam(new BigDecimal(-1), SubProParam.OUT);
		params.add(out);
		try {
			broker2.executeSubPro(SET_NEW_RELEASE, params);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
	...
	public boolean setVisileStatus(int vodid, int subid){
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(vodid), SubProParam.IN);
		params.add(in);
		...
		params.add(out);
		try {
			broker2.executeSubPro(SET_INVI_STATUS, params);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
	...
	public boolean changeSubjectOfSvcVod(int vodid, String subid){
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(vodid), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(subid), SubProParam.IN);
		params.add(in);

		SubProParam out = new SubProParam(new BigDecimal(-1), SubProParam.OUT);
		params.add(out);
		try {
			broker2.executeSubPro(CHANGE_SVC_SUBJECT_VOD, params);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
	...
	public boolean addVodToService(int subId, String vodId){
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(subId), SubProParam.IN);
		params.add(in);
		in = new SubProParam(new String(vodId), SubProParam.IN);
		params.add(in);

		SubProParam out = new SubProParam(new BigDecimal(-1), SubProParam.OUT);
		params.add(out);
		try {
			broker2.executeSubPro(ADD_VOD_TO_SERVICE, params);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
	...
	public boolean removeVodService(int subId, String vodId){
		boolean flag = true;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new BigDecimal(subId), SubProParam.IN);
		params.add(in);
		...
		params.add(out);
		try {
			broker2.executeSubPro(REMOVE_VOD_SERVICE, params);
		} catch (Exception e) {
			flag = false;
			e.printStackTrace();
		}
