...
public class IMBroker {
	...
	{
		if (params == null || params.size() == 0 || prepStmt == null)
			return;
		...
		for (int i = 0; i < params.size(); i++)
		{
			...
			if (param == null)
				throw new SQLException("From IMBroker.setParam(...): parameter is NULL at params[" + i + "]");
			else if (param instanceof java.lang.String)
				prepStmt.setString(i + 1, (java.lang.String) param);
			else if (param instanceof java.math.BigDecimal)
				prepStmt.setBigDecimal(i + 1, (java.math.BigDecimal) param);
			else if (param instanceof java.sql.Timestamp)
				prepStmt.setTimestamp(i + 1, (java.sql.Timestamp) param);
			else
				throw new SQLException("From IMBroker.setParam(...): Invalid parameter type at params[" + i + "]");
		}
	}
	...
	{
		if (params == null || params.size() == 0 || cs == null)
			return;

		for (int i = 0; i < params.size(); i++)
		{
			Object paramObject = (Object) params.get(i);
			if (!(paramObject instanceof SubProParam))
				throw new SQLException("From DBI.registerParams(...): is not SubProParam type at params[" + i + "]");
			...
			{// Set ParamIN
				if (value == null)
					throw new SQLException("From DBI.registerParams(...): parameterIN is NULL at params[" + i + "]");
				else if (value instanceof java.lang.String)
					cs.setString(i + 1, (java.lang.String) value);
				else if (value instanceof java.math.BigDecimal)
					cs.setBigDecimal(i + 1, (java.math.BigDecimal) value);
				else if (value instanceof java.sql.Timestamp)
					cs.setTimestamp(i + 1, (java.sql.Timestamp) value);
				else
					throw new SQLException("From IMBroker.registerParams(...): Unknown parameterIN[" + i + "] type");
			}
			if (type == SubProParam.OUT || type == SubProParam.INOUT)
			{// Register ParamOUT
				if (value == null)
					throw new SQLException("From DBI.registerParams(...): parameterOUT is NULL at params[" + i + "]");
				else if (value instanceof java.lang.String)
					...
					cs.registerOutParameter(i + 1, Types.ARRAY, param.getNameOfTypeArray());
				else
					throw new SQLException("From DBI.registerParams(...): Unknown parameterOUT[" + i + "] type");
			}
		}
	}
	...
	public Vector executeSubPro(String sqlSubPro, Vector params) throws SQLException
	{
		...
		if (params != null && params.size() > 0)
			returnValues = new Vector(params);
		if (sqlSubPro == null)
			throw new SQLException("sqlSubPro == null");
		String strSqlBlock = sqlSubPro.trim();
		if (strSqlBlock.length() == 0)
			throw new SQLException("sqlSubPro.length() == 0");
		try
		{
