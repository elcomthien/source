...
public class ConPool {
	...
	private Connection getConnFromPool() {
		synchronized (mutex) {
			...
			logger.info("ConPool.getConnFromPool = " + conn + " - pool: "
					+ pool.size());
			return conn;
		}
	}
		...
		while (attemptNo < config.conn_tries_numbers) {
			try {
				...
				sconn = config.connectString
						...
						+ config.pass
						+ ""
						+ "&noAccessToProcedureBodies=true&characterEncoding=utf8";
				// sconn =
				// "jdbc:mysql://10.9.1.10:8668/dbadcenter?user=adcenter&password=adcenter&noAccessToProcedureBodies=true&characterEncoding=utf8";
				...
				// retrying
				String msg = "Connect to the Database failed in times: "
						+ attemptNo;
				ex.printStackTrace();
				logger.error(msg, ex);
				try {
