...
//	var str = link.substring(link.lastIndexOf("#")-(-1), link.length);
//	var arr = str.split("-");
//	var datajson = {"idsubject":arr[0],"id":arr[1]};
//	var jdata = JSON.stringify(datajson);
//	alert(jdata);
//	$.post("gettext.elcom", {data : jdata}, function(r) {
//		
//	});
});
$(function(){
...
//$("#edit_text_content").css("height","150").css("width","477").htmlbox({
//    toolbars:[
...
function loadContentText(link){
	var str = link.substring(link.lastIndexOf("#")-(-1), link.length);
	var arr = str.split("-");
	var datajson = {"idsubject":arr[0],"id":arr[1]};
	var jdata = JSON.stringify(datajson);
	alert(jdata);
	$.post("gettext.elcom", {data : jdata}, function(data) {
		alert(data);
	});
}
...
$(document).mousedown(function(event) {
	...
	case 1:
		var link = document.URL;
		loadContentText(link);
	break;
//	case 2:
//	alert('Bạn vừa click chuột giữa');
//	break;
//	case 3:
//	alert('Bạn vừa click chuột phải');
//	break;
	default:
	alert('Bạn có một con chuột khá lạ!');
	}
	});