...
public class eSmileDao {
	...
	@SuppressWarnings("unchecked")
	public List<HashMap<String, String>> getListWelcome(String langid) {
		List<HashMap<String, String>> list = new ArrayList<>();
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new String(langid), 0);
		params.add(in);
		...
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.GET_WELCOME, params);
			if ((params != null) & (params.size() > 0)) {
				subOut = (SubProParam) params.get(1);
				outParam = subOut.getVector();
			}
		...
		LogUtil.logDao(eSmileDao.class.toString(), SQL.GET_WELCOME, params, "langid", outParam.size());
		for (int i = 0; i < outParam.size(); i = i + 2) {
			HashMap<String, String> map = new HashMap<>();
			map.put("id", outParam.get(i));
			map.put("name", outParam.get(i + 1));
	...
	@SuppressWarnings("unchecked")
	public HashMap<String, String> editWelcome(String id, String name, String langid) {
		int rs = -1;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new String(id), 0);
		params.add(in);
		...
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.EDIT_WELCOME, params);
			if ((params != null) & (params.size() > 0)) {
				SubProParam paramOUT = (SubProParam) params.get(3);
				rs = ConvertUtil.convertToInteger(paramOUT.getString().trim());
			}
		...
		LogUtil.logDao(eSmileDao.class.toString(), SQL.EDIT_WELCOME, params, "id,name,langid", rs);
		HashMap<String, String> hm = new HashMap<String, String>();
		hm.put("status", String.valueOf(rs));
		if (rs == 1)
			hm.put("message", "OK");
