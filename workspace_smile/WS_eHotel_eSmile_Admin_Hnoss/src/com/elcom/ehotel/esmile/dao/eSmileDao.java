...
import com.elcom.ehotel.esmile.util.HttpURLConnectionCon;
...
public class eSmileDao {
	...
	@SuppressWarnings("unchecked")
	public HashMap<String, String> getLogout(String userid) {
		int rs = -1;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new String(userid), 0);
		params.add(in);
		...
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.GET_LOGOUT, params);
			if ((params != null) & (params.size() > 0)) {
				SubProParam paramOUT = (SubProParam) params.get(1);
				rs = ConvertUtil.convertToInteger(paramOUT.getString().trim());
			}
		...
		LogUtil.logDao(UserDao.class.toString(), SQL.GET_LOGOUT, params, "userid", rs);
		HashMap<String, String> hm = new HashMap<String, String>();
		hm.put("status", String.valueOf(rs));
		if (rs == 1) {
			hm.put("message", "Logout success");
			try {
				HttpURLConnectionCon.sendPost(userid, "", "", "");
			} catch (Exception e) {
				e.printStackTrace();
			}
		} else
			hm.put("message", "Logout fail");
		return hm;
	}
...
//		System.out.println(e.getDataStandard("24,26,27,1,2,3,4,5,6,7,22", "421,419,420", "22-09-2017 06:00", "29-09-2017 23:00", "1"));
		 ...
		 System.out.println(e.getStatisticSmile("01-10-2017 06:00", "13-10-2017 23:00", "440,421,419,420", "1"));
	}
}
