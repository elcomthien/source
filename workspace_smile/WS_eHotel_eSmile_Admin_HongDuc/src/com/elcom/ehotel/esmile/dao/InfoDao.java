...
public class InfoDao {
	...
	@SuppressWarnings("unchecked")
	public List<SubjectInfoModel> getSubjectInfo(String langid, String id) {
		...
		LogUtil.logDao(InfoDao.class.toString(), SQL.GET_SUBJECT_INFO, params, "langid,id", outParam.size() / 5);
	...
	@SuppressWarnings("unchecked")
	public HashMap<String, String> addSubjectInfo(String name, String image, String id) {
		int rs = -1;
		Vector<SubProParam> params = new Vector<SubProParam>();
		...
		LogUtil.logDao(eSmileDao.class.toString(), SQL.ADD_SUBJECT_INFO, params, "name,image,id", rs);
	...
	@SuppressWarnings("unchecked")
	public List<HashMap<String, String>> getService(String id, String langid) {
		List<HashMap<String, String>> list = new ArrayList<>();
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new String(id), 0);
		params.add(in);
		...
		LogUtil.logDao(InfoDao.class.toString(), SQL.GET_SERVICE, params, "id,langid", outParam.size() / 7);
		for (int i = 0; i < outParam.size(); i = i + 7) {
			...
			map.put("type", outParam.get(i + 5));
			map.put("typename", outParam.get(i + 6));
			list.add(map);
		}
		return list;
	}
	...
	@SuppressWarnings("unchecked")
	public HashMap<String, String> addService(String id, String name, String image, String type) {
		int rs = -1;
		Vector<SubProParam> params = new Vector<SubProParam>();
		SubProParam in = new SubProParam(new String(id), 0);
		params.add(in);
		...
		LogUtil.logDao(eSmileDao.class.toString(), SQL.ADD_SERVICE, params, "fid,name,image,type", rs);
		HashMap<String, String> hm = new HashMap<String, String>();
		hm.put("status", String.valueOf(rs));
		if (rs == 1)
			hm.put("message", "OK");
	...
	@SuppressWarnings("unchecked")
	public List<HashMap<String, String>> getServiceType() {
		List<HashMap<String, String>> list = new ArrayList<>();
		Vector<SubProParam> params = new Vector<SubProParam>();
		Vector<String> outParam = new Vector<String>();
		SubProParam subOut = new SubProParam(outParam, "STRING_ARR", 1);
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.GET_SERVICE_TYPE, params);
			if ((params != null) & (params.size() > 0)) {
				subOut = (SubProParam) params.get(0);
				outParam = subOut.getVector();
			}
		...
		LogUtil.logDao(InfoDao.class.toString(), SQL.GET_SERVICE_TYPE, params, "none", outParam.size() / 3);
		for (int i = 0; i < outParam.size(); i = i + 3) {
			HashMap<String, String> map = new HashMap<>();
			map.put("id", outParam.get(i));
			map.put("name", outParam.get(i + 1));
			map.put("desc", outParam.get(i + 2));
			list.add(map);
		}
		return list;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(i.getSubjectInfo("2", "INFO"));
	}
}
