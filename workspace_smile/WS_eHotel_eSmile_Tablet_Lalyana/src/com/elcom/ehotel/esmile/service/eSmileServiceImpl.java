...
public class eSmileServiceImpl implements eSmileService {
	...
	public String postSurvey(String object) {
		System.out.println("postsurvey: " + object);
		JSONObject jObject;
		try {
			jObject = new JSONObject(object);
			String key = jObject.getString(Param.key);
			...
			String guestname = jObject.getString(Param.nameguest);
			...
			return new Gson().toJson(eSmileDao.setRatingSurvey(key, listPost, folionum, guestname).get(0));
		} catch (JSONException e) {
			e.printStackTrace();
		}
		ObjResult rs = new ObjResult("0", Param.fail);
	...
	public String setRatingGuest(String object) {
		...
		try {
			...
			JSONArray arrayRatingid = jObject.getJSONArray(Param.rating);
			return new Gson().toJson(eSmileDao.setRatingGuest(key, smileid, arrayRatingid, folionum, nameguest, checkindate, checkountdate)
					.get(0));
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public String setCommentGuest(String object) {
		...
		try {
			...
			String checkindate = jObject.getString(Param.checkindate);
			String checkountdate = jObject.getString(Param.checkoutdate);
			return new Gson().toJson(eSmileDao.setCommentGuest(key, smileid, textcomment, folionum, nameguest, checkindate, checkountdate)
					.get(0));
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	@Override
	public String setCommentNew(String object) {
		System.out.println("setCommentNew + object: " + object);
		JSONObject jObject;
		try {
			jObject = new JSONObject(object);
			String key = jObject.getString(Param.key);
			String room = jObject.getString(Param.folionum);
			String guest = jObject.getString(Param.nameguest);
			String textcomment = jObject.getString(Param.text_comment);
			return new Gson().toJson(eSmileDao.setCommentNew(key, room, guest, textcomment));
		} catch (JSONException e) {
			e.printStackTrace();
		}
		ObjResult rs = new ObjResult("0", Param.fail);
	...
	@Override
	public String getNotifyComment(String object) {
		List<ObjNotify> list = new ArrayList<>();
		list = eSmileDao.getNotifyComment();
