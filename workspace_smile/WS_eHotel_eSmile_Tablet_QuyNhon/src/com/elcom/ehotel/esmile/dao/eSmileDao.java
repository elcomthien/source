...
public class eSmileDao {
	...
	public List<ObjLang> getLanguage() {
		...
		if (outParam.size() > 0) {
			...
			for (int i = 0; i < outParam.size(); i = i + 4) {
				ObjLang lang = new ObjLang(outParam.get(i), outParam.get(i + 1), outParam.get(i + 2),
						outParam.get(i + 3));
				list.add(lang);
			}
		}
	...
	public List<ObjSubjectRating> getSubjRating(String langid) {
		...
		if (outParam.size() > 0) {
			...
			for (int i = 0; i < outParam.size(); i = i + 5) {
				ObjSubjectRating objSujRating = new ObjSubjectRating(outParam.get(i), outParam.get(i + 1),
						outParam.get(i + 2), outParam.get(i + 3), outParam.get(i + 4));
		...
		if (input != null && input.length() > 0) {
			...
			if (input.contains("|")) {
				...
				for (int i = 0; i < arr2.length; i += 5) {
					ObjRatingStaff obj = new ObjRatingStaff(arr2[i], arr2[i + 1], arr2[i + 2], arr2[i + 3],
							arr2[i + 4]);
					arrRating.add(obj);
				}
				return arrRating;
	...
	public static void main(String[] args) {
		...
		System.out.println(e.getInfo("2001", "1", "1"));
	...
	public List<ObjSubject> getPmsSubject(String key) {
		...
		if (outParam.size() > 0) {
			...
			for (int i = 0; i < outParam.size(); i = i + 13) {
				ObjSubject subject = new ObjSubject(outParam.get(i), outParam.get(i + 1), outParam.get(i + 2),
						outParam.get(i + 3), outParam.get(i + 4), outParam.get(i + 5), outParam.get(i + 6),
						outParam.get(i + 7), outParam.get(i + 8), outParam.get(i + 9), outParam.get(i + 10),
						outParam.get(i + 11), outParam.get(i + 12));
				list.add(subject);
			}
		}
	...
	public List<ObjItem> getPmsSubjectDetail(String key, String subjectid) {
		...
		if (outParam.size() > 0) {
			...
			for (int i = 0; i < outParam.size(); i = i + 9) {
				ObjItem item = new ObjItem(outParam.get(i), outParam.get(i + 1), outParam.get(i + 2),
						outParam.get(i + 3), outParam.get(i + 4), outParam.get(i + 5), outParam.get(i + 6),
						outParam.get(i + 7), outParam.get(i + 8));
				list.add(item);
			}
		}
		return list;
	...
	public List<ObjItemInfo> getItemBySubjectID(String key, String subjectid, String langid) {
		...
		if (outParam.size() > 0) {
			LogUtil.logDao(eSmileDao.class.toString(), SQL.GET_INTEM_BY_SUBJECT_ID, params, "none",
					outParam.size() / 3);
			for (int i = 0; i < outParam.size(); i = i + 9) {
	...
	public List<SurveyModel> getSurvey(String langid) {
		...
		if (outParam.size() > 0) {
			...
			for (int i = 0; i < outParam.size(); i = i + 2) {
				SurveyModel objSurvey = new SurveyModel(convertToArraySmile(outParam.get(i)),
						convertToArrayRating(outParam.get(i + 1)));
				list.add(objSurvey);
			}
		}
		return list;
	...
	@SuppressWarnings("unchecked")
	public List<ObjResult> setRatingSurvey(String key, String staffid, List<ObjPostSurvey> arrSurveyPost) {
		List<ObjResult> list = new ArrayList<>();
		Vector<SubProParam> params = new Vector<SubProParam>();
		Vector<String> outParam = new Vector<String>();
		SubProParam param = null;
	...
	public List<ObjSubjectDining> getSubjectDining(String key, String langid) {
		...
		if (outParam.size() > 0) {
			...
			for (int i = 0; i < outParam.size(); i = i + 3) {
				ObjSubjectDining objSubject = new ObjSubjectDining(outParam.get(i), outParam.get(i + 1),
						outParam.get(i + 2));
				list.add(objSubject);
			}
		}
	...
	public List<ObjSubjectDining> getItemDiningBySubjectID(String key, String subjectid, String langid) {
		...
		if (outParam.size() > 0) {
			LogUtil.logDao(eSmileDao.class.toString(), SQL.GET_SUBJECT_DINING_BYSUBJECTID, params, "none",
					outParam.size() / 3);
			for (int i = 0; i < outParam.size(); i = i + 3) {
				ObjSubjectDining objSubject = new ObjSubjectDining(outParam.get(i), outParam.get(i + 1),
						outParam.get(i + 2));
	...
	public List<ObjFood> getListFootByItem(String key, String itemid, String langid) {
		...
		if (outParam.size() > 0) {
			...
			for (int i = 0; i < outParam.size(); i = i + 6) {
				...
				ObjFood objFood = new ObjFood(outParam.get(i), outParam.get(i + 1), outParam.get(i + 2),
						outParam.get(i + 3), outParam.get(i + 4), outParam.get(i + 5));
				list.add(objFood);
			}
		}
		return list;
	...
	public List<ObjShipSchedule> getShipSchedule(String key, String langid) {
		...
		if (outParam.size() > 0) {
			...
			for (int i = 0; i < outParam.size(); i = i + 3) {
				ObjShipSchedule objShip = new ObjShipSchedule(outParam.get(i), outParam.get(i + 1),
						convertToArrayTime(outParam.get(i + 2)));
				list.add(objShip);
			}
		}
		return list;
	...
	public JSONObject getRatingNotify() {
		...
		if (outParam.size() >= 7) {
			...
			for (int i = 0; i < outParam.size(); i = i + 7) {
				ObjRatingNotify obj = new ObjRatingNotify(outParam.get(i), outParam.get(i + 1), outParam.get(i + 2),
						outParam.get(i + 3), UnicodeConverter.decodeUnicode(outParam.get(i + 4)), outParam.get(i + 5), outParam.get(i + 6));
				list.add(obj);
			}
		}
	...
	public List<ObjNotify> getNotify() {
		...
		if (outParam.size() >= 9) {
			...
			for (int i = 0; i < outParam.size(); i = i + 9) {
				ObjNotify obj = new ObjNotify(outParam.get(i), outParam.get(i + 1), outParam.get(i + 2),
						outParam.get(i + 3), outParam.get(i + 4), outParam.get(i + 5), outParam.get(i + 6),
						outParam.get(i + 7), outParam.get(i + 8));
				list.add(obj);
			}
	...
	@SuppressWarnings("unchecked")
	public List<ObjResult> setRatingGuest(String key, String smileid, JSONArray arrRatingID, String folionum,
			String nameguest, String checkindate, String checkountdate) {
		List<ObjResult> list = new ArrayList<>();
		Vector<SubProParam> params = new Vector<SubProParam>();
		Vector<String> outParam = new Vector<String>();
		SubProParam param = null;
	...
	@SuppressWarnings("unchecked")
	public List<ObjResult> setCommentGuest(String key, String smileid, String textcomment, String folionum,
			String nameguest, String checkindate, String checkountdate) {
		List<ObjResult> list = new ArrayList<>();
		Vector<SubProParam> params = new Vector<SubProParam>();
		Vector<String> outParam = new Vector<String>();
	...
	public ObjInfo getInfos(String key, String langid, String typein) {
		...
		if (outParam.size() > 0) {
			...
			// for (int i = 0; i < outParam.size(); i = i + 9) {
			int i = 0;
			ObjInfo loc = new ObjInfo(outParam.get(i), convertToArrayLang(outParam.get(i + 1)),
					convertToArraySmile(outParam.get(i + 2)), convertToArrayRatingTablet(outParam.get(i + 3)),
					convertToArrayRatingTablet(outParam.get(i + 4)), convertToArrayPage(outParam.get(i + 5)),
					convertToArrayWelcome(outParam.get(i + 6)), convertToArrayBackground(outParam.get(i + 7)),
					convertToArrayPromotion(outParam.get(i + 8)),convertToArrayRatingDetail(outParam.get(i + 9)));
			return loc;
			// }
		}
		return objInfo;
	...
	public ObjLogin postLogin(String username, String password, String key) {
		...
		params.add(param);
		param = new SubProParam(new String(com.elcom.ehotel.esmile.util.Encryptor.encryptPassword(password)),
				SubProParam.IN);
		params.add(param);
		param = new SubProParam(new String(key), SubProParam.IN);
		params.add(param);
		...
		if (outParam.size() > 0) {
			...
			if (Integer.parseInt(outParam.get(i)) > 0)
				objLogin = new ObjLogin("1", Param.ok, outParam.get(i), outParam.get(i + 1), outParam.get(i + 2),
						outParam.get(i + 3));
			else
	...
	@SuppressWarnings("unchecked")
	public List<ObjResult> postRatingChild(String key, String smileid, String idrating, JSONArray arrRatingID,
			String staffid) {
		List<ObjResult> list = new ArrayList<>();
		Vector<SubProParam> params = new Vector<SubProParam>();
		Vector<String> outParam = new Vector<String>();
	...
	public List<ObjStaff> getEmployee() {
		...
		if (outParam.size() > 0) {
			...
			for (int i = 0; i < outParam.size(); i = i + 6) {
				ObjStaff loc = new ObjStaff(outParam.get(i), outParam.get(i + 1), outParam.get(i + 2),
						outParam.get(i + 3), outParam.get(i + 4), outParam.get(i + 5));
				list.add(loc);
			}
		}
