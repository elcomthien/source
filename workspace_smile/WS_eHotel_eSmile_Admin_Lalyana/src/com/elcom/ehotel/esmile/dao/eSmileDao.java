...
import com.elcom.ehotel.esmile.model.CommentModel;
...
public class eSmileDao {
	...
	public SurveyModel getListSurvey(String langid) {
		...
		for (int j = count * 3 + 1; j < outParam.size(); j = j + 4) {
			...
			map.put("name", UnicodeConverter.decodeUnicode(outParam.get(j + 1)));
	...
	@SuppressWarnings("unchecked")
	public List<HashMap<String, Object>> getVoteSurveyNew(String from, String to, String listroom, String langid) {
		...
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.GET_VOTE_SURVEY_NEW, params);
			if ((params != null) & (params.size() > 0)) {
				subOut = (SubProParam) params.get(4);
				outParam = subOut.getVector();
			}
		...
		LogUtil.logDao(eSmileDao.class.toString(), SQL.GET_VOTE_SURVEY_NEW, params, "from,to,listroom,langid", outParam.size());

		List<RatingSurveyModel> listsur = new ArrayList<>();
...
//			System.out.println(i + " - " + outParam.get(i));
		for (int i = 1; i < outParam.size(); i += count * 7 + 1) {
			listsur = new ArrayList<>();
			survey = new HashMap<String, Object>();
			room = outParam.get(i);
			for (int j = i + 1; j < i + (count * 7); j += 7) {
				RatingSurveyModel sur = new RatingSurveyModel();
				sur.setId(outParam.get(j));
				sur.setName(outParam.get(j + 1));
				sur.setPoor(outParam.get(j + 2));
				sur.setAverge(outParam.get(j + 3));
				sur.setGood(outParam.get(j + 4));
				sur.setExcellent(outParam.get(j + 5));
				sur.setSum(outParam.get(j + 6));
				listsur.add(sur);
			}
			survey.put("room", room);
			survey.put("survey", listsur);
			list.add(survey);
		}
		return list;
	}
	...
	@SuppressWarnings("unchecked")
	public List<CommentModel> getComment(String from, String to, String listroom) {
		...
		params.add(subOut);
		try {
			params = SQL.broker.executeSubPro(SQL.GET_COMMENT_NEW, params);
			if ((params != null) & (params.size() > 0)) {
				subOut = (SubProParam) params.get(3);
				outParam = subOut.getVector();
			}
		...
		LogUtil.logDao(eSmileDao.class.toString(), SQL.GET_COMMENT_NEW, params, "from,to,listroom", outParam.size() / 5);
		for (int i = 0; i < outParam.size(); i = i + 5) {
			CommentModel cmt = new CommentModel(outParam.get(i), outParam.get(i + 1), outParam.get(i + 2), outParam.get(i + 3),
					outParam.get(i + 4));
			list.add(cmt);
		}
		return list;
	...
	public static void main(String[] args) {
		...
		System.out.println(e.getVoteSurveyNew("01/01/2018", "23/01/2018", "3", "2"));
	}
}
